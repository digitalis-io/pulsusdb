...
impl Lexer {
    ...
    ///
    /// PulsusDB patch (docs/decisions/0003): backtick (`` ` ``) strings are
    /// PromQL's raw string literals (mirroring Go's raw string syntax, the
    /// grammar this crate is modelled on) — no escape processing applies
    /// inside them at all, `\` is just an ordinary character. Only `"`/`'`
    /// strings apply escape sequences, so the `symbol != '`'` guard below
    /// is the whole fix: a leaf lexer-state bug (this raw-string scanning
    /// rule unconditionally entered `Escape` state for every `\`,
    /// regardless of delimiter), not a grammar production. See
...
mod tests {
    ...
    fn test_strings() {
        let cases = vec![
            ...
            // PulsusDB patch (docs/decisions/0003): backtick strings are
            // raw strings — no escape processing, so `\.` is two literal
            // characters, not an escape error (was previously asserted to
            // error, matching the pre-patch bug).
