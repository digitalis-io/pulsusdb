...
use crate::parser::token::{T_EQL, T_EQL_REGEX, T_NEQ, T_NEQ_REGEX, TokenId, token_display};
...
impl fmt::Display for Matchers {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if or_matchers.is_empty() {
            // PulsusDB patch (docs/decisions/0003): preserve parse order
            // instead of re-sorting alphabetically by rendered text. The
            // *set* of matchers is unchanged either way, but sorting broke
            // the `parse -> Display -> parse` round-trip invariant (a
            // strict, order-sensitive `Vec` equality) for every selector
            // with 2+ matchers whose parse order wasn't already
            // alphabetical — a leaf `Display` fix. See
            // vendor/promql-parser/PATCHES.md.
            write!(f, "{}", join_vector(simple_matchers, ",", false))
        } else {
