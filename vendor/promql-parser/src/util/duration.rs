...
/// PulsusDB patch (docs/decisions/0003): the largest number of seconds
/// representable as a Go `time.Duration` (`i64` nanoseconds) — Prometheus's
/// own overflow bound for a bare-number (no unit suffix) duration, ~292.47
/// years. A bare number beyond this (e.g. `foo offset 9.5e10`, `foo[9.5e10]`)
/// must be rejected with "duration out of range" to match upstream v3.13;
/// this crate's own `Duration` (backed by `u64` seconds) previously accepted
/// such values without bound, an always-on divergence reachable through both
/// `offset` and the matrix-selector range `[...]`. Applied only to the
/// plain-float-seconds branch below (leaf fix — the unit-suffixed branch's
/// `ALL_CAPS` computation, and its own already-passing tests, are untouched).
const MAX_DURATION_SECS: f64 = i64::MAX as f64 / 1_000_000_000.0;
...
pub fn parse_duration(ds: &str) -> Result<Duration, String> {
    ...
    if let Ok(float_duration) = ds.parse::<f64>() {
        if !float_duration.is_finite() || float_duration.abs() > MAX_DURATION_SECS {
            return Err(format!("duration out of range: {ds}"));
        }
