...
//! | sample fetch — value decode, storage → engine | Covered by: crates/pulsus-read/tests/live_metrics_engine.rs::rate_end_to_end_against_real_samples<br>`Limit:` answer-level, not bit-level. `assert_eq!` on `f64` compares by VALUE, and a passing value comparison is evidence about bits in neither direction: it accepts `+0.0` against `-0.0`, whose bits differ (`0x0000000000000000` vs `0x8000000000000000`), and it rejects a `NaN` against itself, whose bits are identical. The two spellings that do make a check bit-level are `to_bits` and `reinterpretAsUInt64`; a scan for both finds only write-side metric-sample assertions, none read-side. |
