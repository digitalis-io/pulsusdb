...
//! L/M/N/P/S range tables), which neither Rust's `{:?}` escaping nor
//! `char::is_control` reproduces (U+00A0 NBSP and U+00AD soft hyphen
//! must escape as `\u00a0`/`\u00ad`, the zero-width space U+200B as
//! `\u200b`). The tables at the bottom are transcribed verbatim from
//! go1.25.5 `src/strconv/isprint.go` (itself generated by
//! `makeisprint.go`); [`is_print`] mirrors `strconv.IsPrint`'s lookup
//! exactly (even/odd range-pair binary search, the not-print exception
//! lists, the `>= 0x20000` early accept). Parity with the real Go
//! implementation is pinned over the FULL codepoint space —
//! `go_is_print_full_codepoint_space_checksum_matches_go` below.

/// Go `strconv.IsPrint`, exactly: the Latin-1 fast path (only
/// `0x20..=0x7E` and `0xA1..=0xFF` minus the soft hyphen `0xAD` are
/// printable — U+00A0 NBSP is NOT), then the even/odd range-pair binary
...
fn is_print(c: char) -> bool {
    ...
    if r < 0x10000 {
        let rr = r as u16;
        // First index i with IS_PRINT16[i] >= rr — the start (even) or
        // end (odd) of the range pair that might span rr (Go's
        // `bsearch`).
...
/// Go `strconv.Quote`, in Go's own `quoteWith` order: quote/backslash
...
// doc); even/odd pairs are inclusive range starts/ends, the NOT_PRINT
// lists are sorted exception singletons (IS_NOT_PRINT32 entries are
// offset by 0x10000, exactly like upstream).
#[rustfmt::skip]
...
mod tests {
    ...
    /// Full-codepoint-space parity with real Go: iterating every Rust
    /// `char` (all Unicode scalar values — Go's surrogate range skipped
    /// identically on the Go side), the printable set's size and FNV-1a
    /// hash match go1.25.5 `strconv.IsPrint` exactly. Reproduce the
    /// pinned constants with:
    ///
    /// ```go
    /// var count, h uint64 = 0, 0xcbf29ce484222325
    /// for r := rune(0); r <= 0x10FFFF; r++ {
    ///     if r >= 0xD800 && r <= 0xDFFF { continue }
    ///     if strconv.IsPrint(r) {
    ///         count++
    ///         v := uint32(r)
    ///         for i := 0; i < 4; i++ { h ^= uint64(byte(v >> (8 * i))); h *= 0x100000001b3 }
    ///     }
    /// }
    /// // count=148998 hash=0xdcca923bf445d226
    /// ```
    #[test]
    fn go_is_print_full_codepoint_space_checksum_matches_go() {
        let mut count: u64 = 0;
        let mut h: u64 = 0xcbf2_9ce4_8422_2325;
        const PRIME: u64 = 0x0000_0100_0000_01b3;
        for r in 0..=0x0010_FFFFu32 {
        ...
        assert_eq!(count, 148_998, "printable-rune count diverged from Go");
        assert_eq!(
            h, 0xdcca_923b_f445_d226,
            "printable-rune set diverged from Go"
        );
    }

    /// The round-2 review goldens, byte-for-byte from real
    /// `strconv.Quote` output at go1.25.5: NBSP and the soft hyphen are
    /// NOT Latin-1 printable in Go, and the zero-width space (Cf)
    /// escapes too.
    #[test]
    fn go_quote_escapes_nbsp_soft_hyphen_and_zero_width_space_like_go() {
        assert_eq!(go_quote("a\u{a0}b"), r#""a\u00a0b""#, "NBSP");
        assert_eq!(go_quote("a\u{ad}b"), r#""a\u00adb""#, "soft hyphen");
        assert_eq!(go_quote("a\u{200b}b"), r#""a\u200bb""#, "zero-width space");
    }

    /// Spot checks across every branch, each pinned against real Go
    /// output: printable Latin-1/CJK/emoji verbatim, the named escapes,
    /// `\x` for C0/DEL, `\u` for a non-Latin-1 control (NEL), `\U` for a
    /// higher-plane non-printable (tag character U+E0001).
    #[test]
    fn go_quote_matches_go_strconv_quote_across_the_branches() {
        assert_eq!(go_quote("plain"), r#""plain""#);
        assert_eq!(go_quote("q\"b\\s"), r#""q\"b\\s""#);
        assert_eq!(go_quote("\t\n\r"), r#""\t\n\r""#);
        assert_eq!(go_quote("\u{7}\u{8}\u{c}\u{b}"), r#""\a\b\f\v""#);
        assert_eq!(go_quote("\u{1}\u{1f}\u{7f}"), r#""\x01\x1f\x7f""#);
        assert_eq!(go_quote("\u{e9}\u{4e2d}"), "\"\u{e9}\u{4e2d}\"");
        assert_eq!(go_quote("\u{85}"), r#""\u0085""#, "NEL takes \\u");
        assert_eq!(
            go_quote("\u{1F600}\u{E0001}"),
            "\"\u{1F600}\\U000e0001\"",
