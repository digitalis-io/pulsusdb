...
/// resolves to one of two things: the phase-1 index generator, or a
/// projected slot over the span row's own attribute arrays — a predicate
/// column for a phase-2 condition, and the located element's value,
/// number and kind for a `select()`, an aggregate or a comparison
/// operand. A LogQL `| json` label is a name that is not a column and
/// resolves to a JSON extraction over the line. They are one concept.
///
