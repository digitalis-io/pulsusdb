...
/// IS uncovered.
/// **Why the non-mutating stage is a REGULAR-EXPRESSION label filter.**
/// Since issue #544 an equality or inequality over a structured-metadata
/// name compiles into the statement, so `| env = "prod"` leaves the plan
/// with no client-aggregation stage at all and these fixtures would drive
/// nothing. `=~` is refused by that cell — the reference's own regex label
/// matcher disagrees with the pattern it compiles — so it is still
/// evaluated in-engine, and it is still non-mutating, which is the
/// property every row here is about. An `ip()` form is refused on the
/// same ground and, unlike the regular-expression form, compiles to a
/// matcher that retains nothing past the leaf's own result.
