...
///
/// Issue #343 added `b19_offset.test`'s 9 rows. They move the TOTAL and
/// the `derived` exclusion only: `derived` is not a capture claim, so
/// [`PROVENANCE_PERMITS`] does not move, and they are metric queries,
/// which this slice cannot reach in any case.
const TOTAL_DIRECTIVES: usize = 1_209;
...
const EXCLUDED_BY_PROVENANCE: &str = "config-delta file=121, not a capture claim (derived)=23, \
