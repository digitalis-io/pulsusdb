...
//! Three causes, all recorded in
//! `docs/benchmarks/traces-differential-ledger.md`:
//!
//!   * our aggregate runs over the whole trace BEFORE grouping (#492
//!     item 2), so a `by()`-then-aggregate query reports the trace total;
...
//!     compute the true sum (entry 23 of
...
/// cannot drift apart.
const PENDING_492: &str = "our aggregate runs over the whole trace before grouping — \
     ledger row traceql-spanset-aggregate-precedes-grouping, owned by #492 item 2";
...
const MIXED_TYPE: &str = "the reference's mixed-type sum/avg is order-dependent — ledger row \
     traceql-spanset-aggregate-mixed-type-attribute, entry 23 of \
     docs/reference-defects-we-do-not-copy.md";
...
fn fixtures() -> Vec<Fixture> {
    vec![
        ...
        diverges("by_then_count_then_coalesce", "s510a",
            "| by(name) | count() > 1 | coalesce()",
            &["- | m3 | 01,02,03"],
            &["- | m2 | 01,02"],
            PENDING_492),
        // …and an aggregate AFTER the `coalesce()` contributes again.
        parity("coalesce_then_count", "s510a", "| by(name) | coalesce() | count() > 1",
            &["count()=intValue=3 | m3 | 01,02,03"]),
        // The stage-ORDER divergence, pinned rather than hidden: the key,
        // the arm and the position are the reference's; the VALUE is the
        // trace total and there are two groups rather than one.
        diverges("by_then_count", "s510a", "| by(name) | count() > 1",
            &[
                "by(name)=stringValue=alpha,count()=intValue=3 | m2 | 01,02",
                "by(name)=stringValue=beta,count()=intValue=3 | m1 | 03",
            ],
        ...
        // extending.
        diverges("by_agg_by", "s510m", "| by(name) | count() > 0 | by(status)",
            &[
                "by(name)=stringValue=a,count()=intValue=4,by(status)=stringValue=ok | m1 | 01",
                "by(name)=stringValue=a,count()=intValue=4,by(status)=stringValue=error | m2 | 02,03",
                "by(name)=stringValue=b,count()=intValue=4,by(status)=stringValue=ok | m1 | 04",
            ],
            &[
