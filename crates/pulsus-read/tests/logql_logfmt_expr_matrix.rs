    ...
    const SITES: &[(&str, usize, usize, &str)] = &[
        ...
        (
            ...
            "streams :1160, metric :1444 (incl. every binary leaf), the bucketed range \
             read's capability-join fallback :1639, detected_fields :2979, tail :3271 — \
             line numbers current at issue #507, which added the third and found the other \
             four stale by unrelated growth. `POSITIONS` reaches the first TWO and no more: \
