-- case: link_trace_id_eq
-- q: { link:traceID = "000102030405060708090a0b0c0d0e0f" }
...
WHERE date >= toDate('2023-11-14') AND date <= toDate('2023-11-15')
  ...
  AND (key = 'traceID' AND val = '000102030405060708090a0b0c0d0e0f' AND scope = 'link:intrinsic')
GROUP BY trace_id
ORDER BY bound_ts DESC, trace_id ASC
LIMIT 100001

== phase2 hydration (sample batch) ==
WITH arrayFirstIndex((k, s, v) -> k = 'traceID' AND s = 'link:intrinsic' AND v = '000102030405060708090a0b0c0d0e0f', attr_key, attr_scope, attr_val) AS pm0
