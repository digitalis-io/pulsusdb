-- case: link_span_id_eq
-- q: { link:spanID = "0a1b2c3d4e5f6071" }
...
WHERE date >= toDate('2023-11-14') AND date <= toDate('2023-11-15')
  ...
  AND (key = 'spanID' AND val = '0a1b2c3d4e5f6071' AND scope = 'link:intrinsic')
GROUP BY trace_id
ORDER BY bound_ts DESC, trace_id ASC
LIMIT 100001

== phase2 hydration (sample batch) ==
WITH arrayFirstIndex((k, s, v) -> k = 'spanID' AND s = 'link:intrinsic' AND v = '0a1b2c3d4e5f6071', attr_key, attr_scope, attr_val) AS pm0
