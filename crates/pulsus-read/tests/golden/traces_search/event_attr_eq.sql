-- case: event_attr_eq
-- q: { event.exception.type = "IOError" }
...
WHERE date >= toDate('2023-11-14') AND date <= toDate('2023-11-15')
  ...
  AND (key = 'exception.type' AND val = 'IOError' AND scope = 'event')
GROUP BY trace_id
ORDER BY bound_ts DESC, trace_id ASC
LIMIT 100001

== phase2 hydration (sample batch) ==
WITH arrayFirstIndex((k, s, v) -> k = 'exception.type' AND s = 'event' AND v = 'IOError', attr_key, attr_scope, attr_val) AS pm0
