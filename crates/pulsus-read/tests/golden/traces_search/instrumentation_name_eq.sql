-- case: instrumentation_name_eq
-- q: { instrumentation:name = "io.opentelemetry.contrib.http" }

== phase1 generator[0] ==
SELECT trace_id, max(timestamp_ns) AS bound_ts
FROM trace_spans
WHERE timestamp_ns > 1700000000000000000 AND timestamp_ns <= 1700010800000000000
  AND (if(length(scope_name) <= 8192, scope_name, substringUTF8(scope_name, 1, 2048)) = 'io.opentelemetry.contrib.http')
GROUP BY trace_id
