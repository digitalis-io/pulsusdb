...
WHERE date >= toDate('2023-11-14') AND date <= toDate('2023-11-15')
  ...
  AND (key = 'relation' AND val = 'child_of' AND scope = 'link')
GROUP BY trace_id
ORDER BY bound_ts DESC, trace_id ASC
LIMIT 100001

== phase2 hydration (sample batch) ==
WITH arrayFirstIndex((k, s, v) -> k = 'relation' AND s = 'link' AND v = 'child_of', attr_key, attr_scope, attr_val) AS pm0
