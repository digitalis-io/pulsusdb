...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("search_eval.rs", "run_pipeline", "Vec::new", 2,
     "the initial spanset's empty attributes vec (allocates nothing) + the empty vector returned when a stage empties the pipeline (the trace is dropped, nothing is retained)"),
    ...
    ("search_eval.rs", "by_stage", "Vec::with_capacity", 1,
     "one output spanset's attributes at exactly prefix.len + keys.len slots, charged before the reservation and each display/value payload charged before its clone - the same formula attributes_bytes uses, because build_span_set_groups MOVES this allocation into the retained group"),
    ("search_eval.rs", "coalesce_stage", "Vec::new", 1,
     "the merged spanset's empty attributes vec (coalesce drops the group keys; allocates nothing)"),
