...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "hydrate_batch", "HashMap::", 1,
     "issue #557: the fused-value membership map for a probe a projection reads a value from. Empty at construction; every entry is charged MEMBERSHIP_ENTRY_BYTES + the value length in group_hydrated_rows BEFORE the insert — the same bytes the membership read charged per row before the condition moved onto the hydration statement"),
    ...
    ("exec.rs", "batch_attrs", "HashMap::", 6,
     "select-value + issue-#184 trace-context + issue-#351 event/link value maps: entries charged per row during streaming (entry + string lengths), all released with the batch. The event/link map is built ONLY when a leaf compares an event/link intrinsic against another field, and its cardinality is charged rather than capped — a cap would silently change the answer (a span whose 501st distinct event name is the matching one would stop matching), a charge makes a pathological span a clean 422. Issue #510 added THREE maps and no new read, and issue #557 removed the issue-#479 fused membership-value map from this count (7 -> 6): that map is now built in hydrate_batch and filled from the hydration row's own value column: the aggregate value map (which stopped being a .collect when it gained a sibling), plus the stored-TYPE map beside each of the aggregate and select value maps. Each type entry is one Copy-sized StoredType against a key already charged for its value entry in the same row, so the per-row charge covers them at the same MEMBERSHIP/NUM_VALUE_ENTRY_BYTES rate the value maps pay, and all three die with the batch"),
