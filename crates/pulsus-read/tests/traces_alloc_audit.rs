...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "frame_instant", "Vec::new", 2,
     "metrics instant series/samples - one sample per series; series count fixed (quantile), <= 64 (issue #252 histogram) or probe-capped (grouped), same design carve-out as frame_range"),
