...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "attach_range_exemplars", ".collect", 3,
     "issue #477 wave 2: the per-series value-at-bucket lookup maps (one Vec of BTreeMaps over the ALREADY-materialized series, each bounded by MAX_METRICS_POINTS buckets) and the group-value -> series-index map, bounded by the same probe-capped series count; both are built from the framed answer and read no row; same metrics carve-out"),
