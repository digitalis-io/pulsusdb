...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "batch_attrs", "Vec::new", 2,
     "issue #351, the per-span event/link value vecs (text + numeric): CHARGE PRECEDES GROWTH by construction. The read is ONE ROW PER VALUE, so collect_rows_charged charges each value's bytes as it streams and before the row is handed back; Vec::new() itself allocates nothing, and every later push moves a value whose bytes are already charged. This is the shape review forced on the first cut: the groupUniqArray array column it replaced grew ONE ROW without bound, which broke this module's own Layer-1 residual bound (never a-priori row-unbounded) and put the growth ahead of every charge"),
    // ---- search_eval.rs -------------------------------------------------
