...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "attach_range_exemplars", "Vec::new", 1,
     "exemplar list (issue #182 P5) - bounded by the total exemplar budget (issue #477: MAX_EXEMPLARS for the whole response, thinned to it after collection), and DURING collection by EXEMPLAR_COLLECTION_CEILING, which decimate_if_full enforces by halving the list in place (issue #477 wave 2: the statement now returns one row per bucket AND series identity - a log2 duration bucket, or a comparison side and attribute key - so the row count is no longer bounded by the bucket grid alone); outside the search ByteBudget by design"),
    ("exec.rs", "attach_range_exemplars", ".collect", 5,
     "issue #477 wave 2: the per-series value-at-bucket lookup maps (one Vec of BTreeMaps over the ALREADY-materialized series, each bounded by MAX_METRICS_POINTS buckets) plus ONE series-identity index per shape - group value, log2 bucket bound, or (meta type, attribute key) - each bounded by the same probe-capped series count. Exactly one of the three index maps is built per query (the match arms are exclusive); all are built from the framed answer and read no row; same metrics carve-out"),
    ("exec.rs", "thin_collected_exemplars", ".collect", 1,
     "issue #477: the even-stride reduction of an already-materialized exemplar list down to the total exemplar budget - the kept vector is bounded by that budget (MAX_EXEMPLARS), strictly smaller than the list it replaces, and the stride is taken over the WHOLE response so a grouped answer cannot multiply the budget by its series count; same metrics carve-out"),
    ...
    ("exec.rs", "hydrate_batch", ".collect", 2,
     "issue #557: the per-probe fuses-a-value flags and the per-probe membership containers, one entry per INTERNED PROBE and not per row — bounded by the parser's leaf count, built before the read and released with the batch"),
    ("exec.rs", "hydrate_batch", "HashMap::", 5,
     "issue #557's fused-value membership map plus issue #558's four field maps — the select() value and its stored kind, and the aggregate value and its stored kind, one map per INTERNED FIELD. All five are empty at construction; every entry is charged in group_hydrated_rows BEFORE the insert, at MEMBERSHIP_ENTRY_BYTES + the value length for a membership or select entry and NUM_VALUE_ENTRY_BYTES for an aggregate entry — exactly the bytes the two value reads charged per row before issue #558 moved them onto the hydration statement, so the Layer-2 counter sees what it saw before. A kind entry is one Copy-sized StoredType against a key already charged for its value entry in the same row, so the value charge covers it, which is the rate issue #510 established for the same pair"),
    ("exec.rs", "hydrate_batch", "HashSet::", 1,
     "issue #557: the bare key set for a probe no projection needs a value from. Empty at construction; charged per entry at MEMBERSHIP_ENTRY_BYTES in group_hydrated_rows before the insert"),
    ...
    ("exec.rs", "batch_attrs", ".collect", 1,
     "the issue-#184 child-count map entries, charged per row during streaming (CHILD_COUNT_ENTRY_BYTES). Issue #510 turned the aggregate map's .collect into an explicit loop that fills the value map and the STORED-TYPE map together, so it moved to the HashMap:: row below; issue #557 removed the membership set's, because this function no longer reads the membership — it MOVES the set group_hydrated_rows filled"),
    ("exec.rs", "batch_attrs", "HashSet::", 1,
     "issue #557: the empty placeholder left in the membership slot when its filled set is moved out. HashSet::new allocates nothing, and the set it replaces was charged entry by entry in group_hydrated_rows"),
    ("exec.rs", "batch_attrs", "HashMap::", 2,
     "issue-#184 trace-context + issue-#351 event/link value maps: entries charged per row during streaming (entry + string lengths), all released with the batch. The event/link map is built ONLY when a leaf compares an event/link intrinsic against another field, and its cardinality is charged rather than capped — a cap would silently change the answer (a span whose 501st distinct event name is the matching one would stop matching), a charge makes a pathological span a clean 422. Issue #557 removed the issue-#479 fused membership-value map from this count and issue #558 removed the four field maps (6 -> 2): all five are now built in hydrate_batch and filled from the hydration row's own slot arrays, so this function allocates none of them and MOVES what group_hydrated_rows filled"),
    ...
    ("search_eval.rs", "aggregate_value", ".collect", 2,
     "Vec<(f64, StoredType)> buffers covered by the per-trace transients envelope charged before the aggregates loop. Issue #510 widened the element from f64 to (f64, StoredType) so an integer sum can tell whether EVERY contributor was stored int; the envelope charges size_of::<f64>() + a ref + RETAINED_ENTRY_OVERHEAD per matched span, and the tag rides inside that overhead slack"),
    ...
    ("search_eval.rs", "evaluate_batch", "Vec::with_capacity", 2,
     "summaries buffer: base charge (take x size_of<SpanSummary>) before the reservation. Issue #492 item 2 adds the second: the UNION of the surviving spansets, built ONLY when the pipeline left more than one (a flat query and a coalesced one borrow the single survivor's list and allocate nothing), charged overhead + total x ref width into merged_transient before the reservation and released with it"),
    ...
    ("search_eval.rs", "build_span_set_groups", "Vec::with_capacity", 2,
     "retained groups vec (overhead + n x size_of<SpanSetGroup> charged before) + per-group spans vec (take x size_of<SpanSummary> charged before). Issue #492 item 2 moved the partition into by_stage and the attributes vec is no longer allocated here at all: the fold's allocation is MOVED into the group and its already-charged bytes are subtracted from the transient total (attributes_bytes) rather than charged twice; the sum is still exactly groups_retained_bytes"),
    // ---- issue #492 item 2: the ordered pipeline fold ---------------------
    ("search_eval.rs", "run_pipeline", "Vec::new", 3,
     "the initial spanset's empty attributes vec AND its empty grouping-key vec (Vec::new reserves nothing; the #510 merge separated the response attribute list from the grouping key, so the fold's initial state carries two empty vecs instead of one) + the empty vector returned when a stage empties the pipeline (the trace is dropped, nothing is retained)"),
    ("search_eval.rs", "run_pipeline", "Vec::with_capacity", 1,
     "the aggregate stage's rebuilt attributes list for ONE surviving spanset (issue #510's contribution at its issue #492 item 2 position), at exactly len + 1 slots: SpanSetGroup::retained_bytes accounts the slot vec by capacity(), so a push past capacity would double it and stop the charge and the release agreeing. The slots plus the display and value payload are charged into the fold's live total before the reservation, and the superseded list's slots stay charged there until the caller releases the fold's transient total"),
    ("search_eval.rs", "run_pipeline", ".to_vec", 1,
     "the initial spanset's copy of the matched-span REF list: single_spanset_bytes (overhead + one slot + n x ref width) charged into the fold's live total before the copy, released when the next stage supersedes it or when the caller is done with the response"),
    ("search_eval.rs", "by_stage", "Vec::new", 3,
     "the output spanset list + the distinct-tuple order vec + the per-bucket members outer vec: the two partition vecs are covered by the per-input-set n x PER_SPAN_GROUP_TRANSIENT_BYTES envelope charged before the partition loop (the same envelope the pre-#492 grouping used), and each output spanset charges overhead + its own slot + its attribute slots before it is pushed"),
    ("search_eval.rs", "by_stage", "HashMap::", 1,
     "the tuple->bucket index map: covered by the same PER_SPAN_GROUP_TRANSIENT_BYTES envelope (per-entry key header + value + overhead) + the map key's string payload charged before insert; dropped at the end of each input set and released with the stage's total"),
    ("search_eval.rs", "by_stage", "Vec::with_capacity", 1,
     "one output spanset's attributes at exactly input.attributes.len + keys.len slots, charged before the reservation and each display/value payload charged before its clone - the same formula attributes_bytes uses, because build_span_set_groups MOVES this allocation into the retained group. The output spanset's grouping KEY allocates nothing here: resolve_group_tuple's already-charged tuple is MOVED into it"),
    ("search_eval.rs", "coalesce_stage", "Vec::new", 2,
     "the merged spanset's empty attributes vec AND its empty grouping-key vec - coalesce() clears BOTH lists, and Vec::new reserves nothing"),
    ("search_eval.rs", "coalesce_stage", "Vec::with_capacity", 1,
     "the merged span-ref vec at exactly the surviving total: single_spanset_bytes charged before the reservation, released when the next stage supersedes it"),
