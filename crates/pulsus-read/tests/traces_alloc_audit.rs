...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "frame_compare", ".collect", 5,
     "same bound as frame_compare Vec::new: per-(key,value) baseline/selection sample vectors + the fixed well-known-absent key=nil/total vectors (WELL_KNOWN_COMPARE_KEYS, a bounded 25-key constant), all bounded by the probe-capped series count x MAX_METRICS_POINTS buckets. Issue #460 adds the topN kept-value set, one per side per key, bounded by min(top_n, that key's distinct values) and dropped before the next key"),
