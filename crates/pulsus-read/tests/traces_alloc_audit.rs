...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("search_eval.rs", "evaluate_batch", "Vec::new", 2,
     "out vec slots covered by each match's size_of<TraceMatch> base charge + overhead; plus issue #510's EMPTY flat-attributes vec, which allocates nothing (Vec::new does not reserve) and is the arm taken by every grouped query and every query with no aggregate stage"),
    ...
    ("search_eval.rs", "evaluate_batch", "Vec::with_capacity", 3,
     "summaries buffer: base charge (take x size_of<SpanSummary>) before the reservation. Issue #510 adds two: the per-trace typed aggregate-outcome vec, whose slots ride the per-trace transients envelope and whose only heap payload is a duration string charged by go_duration_string's own scalar-render residual; and the FLAT span set's attributes vec, whose overhead + n x size_of<(String, GroupValue)> is charged BEFORE the reservation and each pair's display + payload before its clone - together exactly what flat_attributes_retained_bytes releases"),
    ...
    ("search_eval.rs", "build_span_set_groups", "Vec::with_capacity", 3,
     "retained groups vec (overhead + n x size_of<SpanSetGroup> charged before) + per-group attributes vec (overhead + attr_contributors.len slots charged before, issue #510: EVERY contributing stage, not the by() keys alone) + per-group spans vec (take x size_of<SpanSummary> charged before); together == groups_retained_bytes"),
