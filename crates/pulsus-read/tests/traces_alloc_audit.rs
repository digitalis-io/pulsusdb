...
const ALLOWLIST: &[(&str, &str, &str, usize, &str)] = &[
    ...
    ("exec.rs", "batch_attrs", "Vec::new", 2,
     "issue #351, the per-span event/link value vecs (text + numeric). PEAK LIVE SET, not per-charge coverage: at any instant a co-loaded value is held by exactly (1) its slot in this vec, (2) its span's single map entry, (3) the string payload MOVED out of the decoded row, and (4) the one row the driver has decoded, dropped before the next. EVENT_VALUE_ENTRY_BYTES + the payload upper-bounds (1)+(2)+(3) — the vec slot is charged DOUBLED for a growing Vec's slack, and a whole map entry is charged per VALUE though the map holds one per SPAN — and (4) is the documented one-block Layer-1 residual. The read streams through stream_rows_charged directly into the map, so no second collection exists: the earlier cut used collect_rows_charged and held the row Vec AND the map at once, charged once and live twice"),
    // ---- search_eval.rs -------------------------------------------------
