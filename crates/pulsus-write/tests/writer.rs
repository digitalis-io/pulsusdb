...
use pulsus_write::{AdmitRefusal, LogRow, LogSink, ParsedLogs, StreamRow};
...
async fn admission_is_rejected_once_shutdown_has_begun() {
    ...
    let result = writer.admit(batch_for(1, "svc", 0, false), PushHeaders::default());
    assert_eq!(result, Err(AdmitRefusal::Backpressure));
}
...
async fn retryable_pre_send_failure_resends_the_whole_batch_before_succeeding() {
    ...
    let wait = writer
        .admit_flush(batch_for(42, "svc", 0, false), PushHeaders::default())
        .expect("queue has room");
    let result = tokio::time::timeout(Duration::from_secs(5), wait)
        .await
...
async fn retryable_pre_send_failure_resends_the_whole_multi_row_batch_before_succeeding() {
    ...
    writer
        .admit(batch_for(101, "svc", 0, false), PushHeaders::default())
        .expect("queue has room");
    writer
        .admit(batch_for(102, "svc", 0, false), PushHeaders::default())
        .expect("queue has room");
    let wait = writer
        .admit_flush(batch_for(103, "svc", 0, false), PushHeaders::default())
        .expect("queue has room");
...
async fn newer_entry_enqueued_during_inflight_backfill_survives_a_stale_success() {
    ...
    let wait = writer
        .admit_flush(batch_for(41, "svc", t1, true), PushHeaders::default())
        .expect("queue has room");
    tokio::time::timeout(Duration::from_secs(60), wait)
        .await
        .expect("flush settles within the test timeout")
    ...
    let wait = writer
        .admit_flush(batch_for(41, "svc", t1 + 1, true), PushHeaders::default())
        .expect("queue has room");
    tokio::time::timeout(Duration::from_secs(60), wait)
        .await
        .expect("flush settles within the test timeout")
...
async fn newer_entry_enqueued_during_inflight_backfill_survives_a_stale_failure() {
    ...
    let t1 = 1_700_000_000_000_000_000i64;
    let wait = writer
        .admit_flush(batch_for(42, "svc", t1, true), PushHeaders::default())
        .expect("queue has room");
    tokio::time::timeout(Duration::from_secs(60), wait)
        .await
        .expect("flush settles within the test timeout")
    ...
    let wait = writer
        .admit_flush(batch_for(42, "svc", t1 + 1, true), PushHeaders::default())
        .expect("queue has room");
    tokio::time::timeout(Duration::from_secs(60), wait)
        .await
        .expect("flush settles within the test timeout")
...
async fn all_distinct_template_batch_balances_queue_bytes_to_zero() {
    ...
    writer
        .admit(pattern_batch(9, "svc", 0, &refs), PushHeaders::default())
        .expect("queue has room");
    writer.shutdown(Duration::from_secs(2)).await;
