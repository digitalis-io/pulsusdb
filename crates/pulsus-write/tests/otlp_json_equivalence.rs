...
fn metrics_self_round_trip_is_row_identical() {
    ...
    assert_parsed_metrics_eq(
        &otlp_metrics::parse(&via_json, NOW_NS, MetricIngestSettings::default())
            .expect("within the expansion budget"),
        &otlp_metrics::parse(&via_pb, NOW_NS, MetricIngestSettings::default())
            .expect("within the expansion budget"),
    );
}
...
fn non_finite_metric_json_decodes_and_preserves_values() {
    ...
    let out = otlp_metrics::parse(&via_json, NOW_NS, MetricIngestSettings::default())
        .expect("within the expansion budget");
