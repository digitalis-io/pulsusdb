...
|-----------|-------------|--------------------|
| **M0 — Foundation** | workspace, config loader, ClickHouse client + pool (benchmarked choice), schema controller with logs+metrics DDL, health endpoints, CI, docker-compose dev env (incl. OTel Collector) | `pulsusdb --mode init` creates a correct schema on a fresh ClickHouse; e2e harness skeleton green |
