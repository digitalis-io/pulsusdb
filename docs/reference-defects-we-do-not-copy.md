...
| [23](#23-a-groups-sum-and-avg-over-one-attribute-give-different-answers-depending-on-which-span-arrived-first) | Tempo | queries | C, D | A group's `sum` and `avg` over one attribute give different answers depending on which span arrived first. |
...
## 23. A group's `sum` and `avg` over one attribute give different answers depending on which span arrived first
