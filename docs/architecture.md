...
- **Traces:** span, resource, and instrumentation-scope attribute keys are stored **verbatim** in the attribute index — TraceQL addresses them by their OTel names (`span.http.status_code`, `resource.service.name`, `instrumentation.<key>`). The index carries a `scope` discriminator (`'resource'`, `'span'`, or `'instrumentation'` — the last added by issue #192) alongside each verbatim `(key, val)` pair, so `resource.foo`/`span.foo`/`instrumentation.foo` remain separable even when the same key appears at multiple scopes.
