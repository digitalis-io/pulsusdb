...
| ClickHouse client | **`clickhouse`** (HTTP + RowBinary) — chosen in M0 (spike: [ADR 0001](decisions/0001-clickhouse-client.md)). Bulk insert 0.86M rows/s (log) / 0.39M rows/s (metric), streaming fetch 0.25M rows/s at bounded RSS; round-trips fingerprints past the signed boundary and `SimpleAggregateFunction`/`AggregateFunction` states correctly — measured at `UInt64` in the spike and re-measured on the `UInt128` column after issue #498, both crates exact. `klickhouse` (native TCP) rejected on insert throughput (46–53% slower on both row shapes) despite passing every correctness/DDL gate. Fallback: `klickhouse`, already proven correct and DDL-capable in the spike; the wrapper's public API (`ChConnConfig`/`ChPool`/`ChClient`/`ChError`) is crate-agnostic so swapping is an internals-only change |
...
| Hashing | `xxhash-rust` and **`ch_cityhash102`** — since issue #498 both halves of every 128-bit fingerprint, in both families; `ch_cityhash102` was selected in M0 by the live length-class parity gate against `SELECT cityHash64(...)` (the only candidate bit-identical to ClickHouse's frozen CityHash 1.0.2 across all input-length branches; `cityhasher` implements upstream 1.1 and diverges) |
...
**It was 64 bits until issue #498**, where a 64-bit identity was carried with no comparison of the
underlying label set: two label sets whose hash collided shared one identity, and the loser's rows
became unreachable rather than mislabelled — no error, no warning, no partial marker. Widening
removes the defect rather than detecting it; there is no collision detector.

Both halves remain bit-identical to a ClickHouse function, so the value stays derivable server-side:
`bitShiftLeft(toUInt128(cityHash64(buf)), 64) + toUInt128(xxHash64(buf))`, pinned against a live
server by `crates/pulsus-model/tests/live_cityhash.rs`.
...
- **Logs:** an OTLP resource or scope attribute key, and a structured-metadata name on either push encoding, is stored under the reference's label name (issue #507): ASCII letters and digits are kept, every run of other characters (`_` included) becomes one `_`, a name of four or more bytes that starts and ends with `__` keeps those two affixes, and a result that starts with a digit gains `key_` (OTel `service.name` → label `service_name`, `k8s.pod.name` → `k8s_pod_name`, `a..b` and `a__b` → `a_b`, `9zone` → `key_9zone`, `--error--` → `_error_`). An empty name, or one that leaves nothing but underscores, is refused ([api.md §8.2](api.md)). An OTLP resource attribute other than `service.name` whose stored name would be `service_name` is stored as `service_name_extracted`. A stream label pushed on the push transport is not renamed: its name must already match `[a-zA-Z_][a-zA-Z0-9_]*`, and it is stored as sent. Names are settled **before** fingerprinting, so a stream's fingerprint is computed over its stored names.
- **Metrics:** attribute keys are escaped one character at a time — each character outside `[a-zA-Z0-9_]` becomes its own `_`, so `a..b` stays `a__b`, unlike the log rule above — and the OTLP metrics receiver follows **Prometheus v3.13.0's own OTLP translation** rather than the log convention (issue #461): a key that would start with `_` or a digit gains the reference's `key`/`key_` prefix, keys that sanitize alike merge into one label with `;`, and the resource's identifying attributes become `job`/`instance` while the rest become a `target_info` series instead of per-series labels. A metric name gains its unit and type suffixes. The rule is selectable with `PULSUS_OTLP_TRANSLATION_STRATEGY` and defaults to the reference's own default. Remote write arrives already translated, so both transports converge on the same series identity — see `crates/pulsus-write/tests/cross_transport_parity.rs`.
- **Traces:** span, resource, instrumentation-scope, span-event, and span-link attribute keys are stored **verbatim** in the attribute index — TraceQL addresses them by their OTel names (`span.http.status_code`, `resource.service.name`, `instrumentation.<key>`, `event.<key>`, `link.<key>`). The index carries a `scope` discriminator (`'resource'`, `'span'`, `'instrumentation'`, `'event'`, `'link'`, or the reserved `'event:intrinsic'` / `'link:intrinsic'` — all added by issue #192) alongside each verbatim `(key, val)` pair, so `resource.foo`/`span.foo`/`instrumentation.foo`/`event.foo`/`link.foo` remain separable even when the same key appears at multiple scopes. Span-event `name`/`timeSinceStart` and span-link `spanID`/`traceID` intrinsics live under their dedicated `event:intrinsic`/`link:intrinsic` scopes so a sender attribute can never collide with an intrinsic row.
- **The promoted physical column is named `service` on the tables whose read paths are service-led — logs, traces, and profiles** — populated from `service.name` (resource) at ingest. It is an internal projection: users see the `service_name` label (logs/profiles) or `resource.service.name` (TraceQL); the planner maps both to the column. **Metrics deliberately have no `service` column**: the hot table stays `(metric_name, fingerprint, unix_milli, value)`, reads are metric-name + fingerprint driven, and service exists there only as the derived `job` label (issue #461; it was the normalized `service_name` label before that).
...
- **Body skip indexes.** `|= "substring"` and `|~ "regex"` filters translate to `body LIKE '%substring%'` / `match(body, regex)` so ClickHouse skips granules that cannot match, instead of scanning every log line in the time range. The planner mints one predicate and adds no prefilter of its own — ClickHouse's own index analysis picks up the ngram bloom for both forms, and the token bloom where the pattern itself delimits a token. Filters that neither index can serve (needles shorter than the 4-gram order, complex regexes) degrade gracefully to a scan *bounded by the stream selector*.
...
Span payload table (`trace_spans`, ordered `(trace_id, timestamp_ns)` with `service_time`, `name_time` and `span_name_day` projections — none of which stores the payload), attribute index (`trace_attrs_idx`, ordered `(key, val, scope, timestamp_ns, trace_id, span_id)` with a typed `val_num` column for numeric comparisons and a `scope` discriminator), and a bounded, scope-aware `trace_tag_catalog` for tag APIs (full DDL and read-path SQL in [schemas.md §4](schemas.md)).

Decisions and their reasons:

- **Four access patterns served by one table.** Primary ordering `(trace_id, timestamp_ns)` makes trace-by-ID fetch a point read; the `service_time` **projection** gives service + time-range searches their own physically sorted copy, so "recent traces for service X" never scans by trace ID; the `name_time` **projection** (issue #555) does the same for span-name searches, which neither the base order nor `service_time` can prune; and the `span_name_day` **aggregate projection** (issue #478) holds one row per `(UTC day, span name)`, so the tag-values Span Name dropdown reads the distinct names rather than every span in the window. Projections cost write amplification; that trade is accepted because trace search is the human-facing slow path, and the aggregate one is small by construction. Neither re-sorted copy stores the `payload` column (issue #555): the only read that selects it filters on `trace_id`, which the base table already orders by, so a second stored copy of the payload was paying for nothing.
...
- **The span row carries its own attributes, and every phase-2 attribute read comes off them (issues #556, #557, #558).** Five aligned arrays on `trace_spans` — `attr_key`, `attr_scope`, `attr_val`, `attr_type`, `attr_num` — hold exactly the attributes that also become `trace_attrs_idx` rows for that span, in the same order. The batch hydration statement reads them through one projected SLOT per condition and per projected field: `arrayFirstIndex` over `(attr_key, attr_scope)` locates the element the span's attribute resolves to, and the condition's value test, the rendered value, its numeric reading and its stored kind are all taken at that one element. `trace_attrs_idx` still serves **phase 1** — the candidate generator — and nothing else on the search route: the `select()`/aggregate/`by()` value reads moved onto the span row in issue #558, and so did the event/link value sets, which expand a span's own array over the rows the reader retained.
- **Two derived tables answer the two searches a search form sends first (#560).** `trace_recent` (one row per five-minute bucket and trace, carrying the newest and the oldest span time) and `trace_error_spans` (one row per span with `status_code = 2`) are written by views over `trace_spans`. The empty search `{}` — and every selector whose only candidate generator is the time-range superset — reads `trace_recent`, and `{ status = error }` reads `trace_error_spans`, instead of scanning the span table, whose `(trace_id, timestamp_ns)` order prunes nothing inside a day partition on a time-only predicate. Both carry their own block-deduplication window, and the span inserts pin the two insert settings that let the window recognise a repeated identical span block ([schemas.md §4.1](schemas.md)).

**A failing view (#560).** The insert fails: a view that throws on a `trace_spans` block makes the caller receive `HTTP 500` with `Code: 395`, and nothing tells the caller which of the span table and its three views' targets kept the block's rows — measured over 300 trials, only the throwing view's own target is reliably empty. Nothing replays the block. With the two derived tables that answer `{}` and `{ status = error }`, this now means a missing answer rather than a missing index row; the contract and its measured distribution are in docs/schemas.md §4.1.
...
- **Errors are per-batch atomic**: a failed insert retries with exponential backoff and jitter; poison batches (schema mismatch) are dumped to a local spool file and skipped, with a counter exposed on `/metrics`. Every spool write is best-effort: a spool I/O failure is logged and counted (`spool_write_failures_total`) and the batch settles anyway, so no spool record is ever guaranteed to exist.
- **Registration backfill** (issues #134/#139): a registration flush that fails *definitely* (`Poisoned` — provably not-committed: either a non-retryable error, or a pre-send retryable exhausted; every post-send retryable is downgraded to `InsertUncertain` before it reaches the writer) enqueues its rows into a bounded in-memory backlog that a dedicated registration backfill task re-inserts every 5s (`REGISTRATION_BACKFILL_RETRY_INTERVAL`; byte cap `REGISTRATION_BACKFILL_MAX_BYTES` = 32 MiB, keyed dedup on `(fingerprint, month)`) until confirmed, with success-only `StreamLru` promotion on a confirmed heal — closing the "sample committed, registration lost" orphan window while the process lives. **The issue-#9 "`InsertUncertain` is never replayed" invariant is not engaged**: uncertain-fate generation failures never enter the backlog (the hook exists only in the Poisoned settlement arm), and a backfill re-insert that itself returns `InsertUncertain` is terminally abandoned, never retried. Re-insertion into `log_streams` is idempotent (`ReplacingMergeTree(updated_ns)`; its only MV is a pure `ARRAY JOIN` fan-out — nothing downstream aggregates), so even a false-Poisoned duplicate collapses on read. Honest residuals (each conditioned on the best-effort spool above): **R1** — an uncertain-fate registration whose samples committed is not healed (an `uncertain/` audit record exists iff that batch's spool write succeeded; audit-only per #9); **R2** — a backfill re-insert returning `InsertUncertain` is terminally abandoned (counted + warn-logged); **R3** — the backlog is memory-only: an orphan persists across a crash only if the stream also never pushes again in that month (a restart empties the LRU, so any later push re-registers naturally); a poison-spool repair record at `{spool}/poison/log_streams/` exists **only when the spool write succeeded**, and manually re-inserting a poison-spooled `log_streams` batch is safe (definitely-not-committed + `ReplacingMergeTree` collapse) — unlike `uncertain/`, which stays audit-only; **R4** — byte-cap drops under sustained failure are counted (`backfill_dropped_total`); a poison-spool record for the dropped batch exists iff that batch's spool write succeeded; **R5** — on spool-write failure there is no durable record: the in-memory backlog is the only (best-effort) remedy, surfaced via the error log + `spool_write_failures_total`; **R4∧R5 compound** — a byte-cap-dropped entry whose generation's spool write also failed is lost entirely (counters and logs are the sole evidence) — acknowledged-lost by design, never claimed healed; the same compound applies to every backlog exit without heal (deterministic/uncertain tick abandons ∧ R5). Healing R1/R2 would require an explicit #9 amendment (an idempotent-table carve-out slotting into the Uncertain settlement arm and the tick's `InsertUncertain` classification) — an owner decision, not implemented.
  One generic mechanism (issue #139) serves all four registration tables, each with its own bounded backlog (the 32 MiB cap is per-backlog; 4 backlogs ⇒ 128 MiB worst-case process-wide) and per-family key/version/idempotency:
  - **`log_streams`** — key `(fingerprint, month)`, version `updated_ns`; idempotent via `ReplacingMergeTree(updated_ns)` (above, unchanged #134 semantics). Heal promotes the success-only `StreamLru` (a pure membership set — safe under any eviction interleaving).
  - **`metric_series`** — key `(metric_name, fingerprint, activity-bucket, value_type)`, **versionless** (the key determines the full logical row; a 128-bit fingerprint collision is not a case this design handles). Plain `MergeTree` but duplicate-tolerant by design: every read-side consumer dedups with `LIMIT 1 BY metric_name, fingerprint`, and the writer already re-emits on an LRU false miss — backfill duplicates are bounded (one per poisoned generation per key) and collapse at read. Heal promotes the success-only `SeriesLru` (membership set — safe).
  - **`metric_metadata`** — key `metric_name`, version `updated_ns`; idempotent via `ReplacingMergeTree(updated_ns)` (a stale re-insert deterministically loses to a larger `updated_ns`). **The heal hook only ever *invalidates* the `MetadataCache` entry — it never installs a descriptor**: the backfill task confirms out of order with the flush task, and after an LRU eviction there is no stored version left to gate on, so any upsert-like heal (plain or version-gated) could resurrect a stale descriptor over the durable winner and permanently suppress a client revert while the DB serves the newer row. Post-heal the cache asserts nothing for the name, so the next admission's decision degrades to *emit* — the safe direction; the redundant row collapses under `ReplacingMergeTree(updated_ns)`.
  - **`trace_attrs_idx`** — key = the full `ReplacingMergeTree` ORDER BY tuple `(key, val, scope, timestamp_ns, trace_id, span_id)`, **versionless** (the non-key columns are deterministic functions of the same attr record/span — a re-insert is the same logical row, `FINAL` collapses to 1). No cache, no heal hook. This closes the "span fetchable by ID but invisible to attribute-scoped search" orphan; the pulsus-write live proof probes the index's own primary-key access path, and the full attribute-scoped TraceQL-**executor** end-to-end proof (issue #167) lives in pulsus-read's live suite — `crates/pulsus-read/tests/traces_search_heal_live.rs` replays the same deterministic failure injection and proves the healed span is found by the real parse → plan → search pipeline (a dev-only test dependency on pulsus-write; the production crate graph is unchanged — pulsus-write still never depends on the executor).

  The append-only tables (`log_samples`, `metric_samples`, `metric_hist_samples`, `trace_spans`, and every rollup/aggregating target) are structurally excluded — their flush contexts carry no Poisoned hook, and each typed backlog is bound to exactly one registration table, so cross-table replay is unrepresentable (#9 applies in full). The residual classes R1–R5 and the R4∧R5 compound above apply per family verbatim (per-family spool directories `{spool}/poison/{table}/`, per-family `BackfillMetrics` counters).
...
**Full PromQL compliance is a hard product requirement, and it does not depend on ClickHouse.** ClickHouse's own PromQL support covers only a small function subset and is never used. **Four aggregations are compiled into SQL, and no others** (issue #549): `min`, `max`, `count` and `group` over a plain instant selector become one statement per fingerprint chunk that returns the answer already reduced. They are compiled because each is exactly reproducible in a statement with no runtime condition, and because the reference's own extremum rule — replace the accumulator when the comparison says to **or when the accumulator is NaN** — is written out in that statement rather than delegated to ClickHouse's `max`, which answers differently: over `[NaN, 1, 3]` it answers `nan` where we answer `3`. Every other PromQL function and operator evaluates in the Rust engine from fetched samples, and 100% of PromQL evaluation semantics live there.
...
- **Compliance gate:** the upstream data-driven PromQL test corpus (21 scenario files, ~11.7k lines covering aggregators, functions, operators, selectors, subqueries, `@`/`at_modifier`, staleness, range queries, duration expressions, and native histograms) is **fetched at test time from the pinned upstream commit, verified against a committed SHA-256 manifest, cached locally, and replayed against PulsusDB in CI**. Full compliance means 100% pass; any temporary exclusion is an open issue, never a silent skip. On each reference-version bump the committed checksum manifest is re-pinned (tag, commit SHA, per-file SHA-256/line counts) — no corpus files are re-vendored.
...
- **Parser:** the `promql-parser` crate is validated against the upstream parser test suite at the pinned version (docs/decisions/0002); the fallback the grammar's non-negotiability implies is **patch-and-vendor**, not necessarily a from-scratch port — issue #31/docs/decisions/0003 found the M2-subset gap concentrated in 5 leaf-level lexer/`Display` bugs (reserved-keyword lexing, raw-string escape handling, a duration-literal overflow bound, two `Display` round-trip bugs), none touching a grammar production, so `promql-parser 0.10.0` is vendored under `vendor/promql-parser/` with those 5 fixes and wired via `[patch.crates-io]` (every `promql_parser::...` import path unchanged). The upstream parser test corpus + M2 AST golden matrix (#29) remains the drift net regardless of which parser sits underneath, gating in CI; a full from-scratch port is only reconsidered if a future re-vendor finds the patch surface growing unboundedly (docs/decisions/0003's re-vendor rule).
...
4. **Fetch** minimal columns over the native protocol:
   ...
   **A pushed query has a different step 4 and no step 5** (issue #549). `min`, `max`, `count` and `group` over a plain instant selector send ONE statement per chunk instead of this fetch and its complementary histogram fetch, and that statement returns one row per **run** — a stretch of consecutive grid points over which the group's answer does not change — rather than one row per stored sample. The reduction has already happened when the rows arrive, so there is nothing for step 5 to evaluate; the reader folds the runs across chunks and the answer is complete. The group key never enters the SQL: group ids are assigned in this process by the same function the evaluator's own aggregation uses, and reach the statement as a parallel array. Everything else in this list is unchanged, including step 3's one resolve.
...
- **Erratum (issue #33 architect adjudication, superseding the original #31 AC):** `count`/`group` aggregations over label matchers alone were originally answered from the label cache with zero ClickHouse queries when the evaluation window lay inside the cache window. This is **withdrawn** — the label cache resolves series presence at activity-*bucket* granularity (`PULSUS_CACHE_WINDOW`'s underlying bucket, §5.2; 1h at M2 defaults), which structurally cannot distinguish "had a sample within the PromQL 5-minute staleness lookback" from "active somewhere in an up-to-24h-old 1-hour bucket". `count`/`group` are exact PromQL aggregation, not an approximable value-matrix construct (§5.1's "100% of semantics in Rust, Prometheus-exact"), and the differential harness (issue #33) proved the divergence live: `count(mem_usage_bytes{service="svc-0"})` returned 69 (including 12 series silent for over 5 minutes) where Prometheus correctly returned 57. `count`/`group` — instant and range alike — resolve → read `metric_samples` → answer, and the lookback is applied in full either way. **Issue #549 amends the SHAPE of that read without touching the rule**: over a plain instant selector, `count`/`group` (and `min`/`max`) now compile into one statement per fingerprint chunk that applies the 5-minute lookback in SQL — a sample covers the grid points from its own timestamp until the next sample of its series or the lookback expiring, whichever comes first — so the answer is the evaluator's, computed from real samples rather than from bucket-granularity activity. What remains withdrawn is answering from the **label cache**, which is what could not be lookback-correct. A lookback-*recency*-capable cache design (tracking each series' own last-sample time, not bucket-floored activity) could restore a cache fast path safely — that is an M3+ evolution of §5.2's cache, not an M2 knob, and is not currently planned.
...
2. **Sample retrieval:** time-bounded read of `log_samples` for the resolved fingerprints; line filters (`|=`, `!=`, `|~`, `!~`) push down as `LIKE`/`match` predicates backed by the body skip indexes.
3. **Pipeline evaluation** in the engine: parsers (`json`, `logfmt`, `regexp`, `pattern`), label filters, `line_format`/`label_format`, unwrap, and range/vector aggregations. One label filter is the exception (issue #544): an equality or inequality over a **structured-metadata** name compiles into the sample statement's `WHERE` and the request `LIMIT` compiles with it, so the read is one statement rather than a page loop — unless the rendered fragments exceed `MAX_METADATA_FRAGMENT_BYTES`, or a selected stream carries both the filter's name and that name without its `_extracted` suffix as labels — in either case the query takes the route it takes today, with the same answer. Every other label filter still evaluates in the engine. Range aggregations that reduce to counting (`rate`, `count_over_time`, `bytes_rate`, `bytes_over_time`) and don't depend on pipeline-created labels are rerouted to the log rollup table when the step is a multiple of its configured resolution.
...
- Span-attribute conditions generate phase-1 candidates from the index, and are then decided in phase 2 by a predicate column on the statement that hydrates the batch — `arrayFirstIndex` over the span row's five attribute arrays, then the value test applied to the element it lands on (issue #557). Intrinsics (`duration`, `status`, `name`, `kind`) filter directly on indexed columns. `service` is not a TraceQL intrinsic — it is the `resource.service.name` attribute, which the planner maps to the physical `service` column.
...
- **Pipeline stages** (spanset aggregates, `by()`, `select()`) are evaluated in the engine, with one exception: a `min(duration)`, `max(duration)` or `count()` aggregate over a selector that is one attribute-equality condition compiles into the phase-1 generator statement's `HAVING` (issue #492 part 4). Which of the rest compile into the SQL instead, and where the boundary between compiled and evaluated falls, is [Query lowering](query-lowering.md). The SQL shape several lowered stages compose into is [ADR 0008](decisions/0008-sql-composition-for-lowered-pipelines.md).
...
- **(P) — decidable from the inputs.** The refusal is a function of data the caller has already materialised, needing auxiliary scratch whose size is a function of INPUT COUNTS alone and never of the output's shape, and constructing no part of the charged output. It must be decided before the stage's charge — subject to the two carve-outs below: one because the charge cannot preempt it there, one because the input is beyond what the stage would admit anyway — and **its own scratch is charged first**, sized from counts read without allocating (`Vec::len()`). That preflight charge is levied against a **ceiling of its own** and never against the caller's counter — a shared counter would make the preflight's admission a function of an unrelated caller's spending, which is the composition hazard the whole rule exists to remove. Assert the domination as a `const`: the preflight's per-series constant plus its per-allocation floors must not exceed the stage model's per-series coefficient.
  - **(P0)** — the part that needs no input-scaled scratch at all runs above even the preflight charge, so it can never be preempted, at any cap, with any counter. (It may still build the refusal message it returns; what it must not allocate is anything whose size an input can influence.)
  - **When the scratch would exceed its ceiling the preflight is SKIPPED, not refused.** A skip is not an `Err`, there is nothing to swallow, and behaviour falls back byte-for-byte to the un-preflighted path — *including the part of that behaviour the preflight exists to correct*: on a skip the reservation answers first again. A preflight that could *refuse* would add a rejection surface instead of correcting one. Size the ceiling so the skip is unreachable for any input the stage itself can admit, and keep a test that reproduces the skipped ordering, so what a skip costs stays visible rather than being described as "unchanged".
  - **Run the preflight only when the reservation it precedes would actually refuse.** The preflight corrects exactly one thing: a resource rejection answering in place of a semantic one. Where the reservation admits there is no rejection to preempt, and the stage raises the same semantic error itself — so skipping cannot change an answer, and the common case pays nothing. Two conditions keep that from becoming the mistake it replaces. The guard must test *the reservation itself*, never a property of the request that merely correlates with it (gating on "does the query use this feature" leaves a low cap or a non-zero shared counter walking straight through). And it must be the *same expression* the reservation refuses on, so the two cannot drift apart. Note the direction of the dependency: this guard may read the caller's counter, because more spending only moves it towards running the preflight. The preflight's own scratch admission still must not.
...
**The worked (P) case.** `post_agg::combine_binary_capped` (issue #290). All five of the LogQL binary funnel's semantic refusals are class (P), and the ordering is structural rather than conventional: `decide_binary` MOVES both operands — and the operator and matching clause they are decided under — into a value with no accessor; `Ledger::acquire_binary` is the only way to get any of them back, and it hands back nothing loose either — its whole result is a second such value carrying the charge as well, which the join must be given BY VALUE and from which it takes every argument but `return_bool`. An operand therefore cannot reach the join except through a charge the decision preceded, and the call to the join cannot supply a matcher, an operator or a budget the operands were not decided and charged under; skipping the decision, conditioning it, deciding one pair and charging another, swapping a decision's halves, and every argument route for substituting the join's context are compile errors rather than review findings.

**Passing the context loosely was not enough, and the reason generalises.** The context stayed substitutable through two rounds because it was `Copy` and travelled as ordinary arguments: shadowing the caller's bindings with the decision's hid the substitution, but renaming the decision's brought it straight back and compiled. `Copy` is not what makes a value substitutable — being *nameable at the call site* is. A guard of this kind is only as strong as its narrowest carrier: put the whole context in one value that only the charge can mint and that is consumed by a single move, and there is nothing left for a caller to pass instead.

**What a carrier of that kind bounds, and where Rust stops** (rounds 4 and 6's `[medium]`). The bound is on the **call**: no caller can substitute the context *by calling the join*, every argument route into it being a compile error (measured on the tree — `E0061`, `E0616`, `E0451`). It is not a bound on the module around it. The carrier's consuming exit is `pub(super)` and its fields are private to the budget module, so code elsewhere in the same file can unpack a carrier and join under its own context without calling the join. That cannot be narrowed further where the carrier lives: a visibility may name only an **ancestor** of the item's own module, so an exit defined in the budget module can be restricted to that module or to one of its ancestors and to nothing else (both measured: naming a sibling module is `E0742`; making the exit private stops the join itself compiling, `E0624`). Moving the join dispatch inside the budget module so the exit could be private is not taken: it would put result-shape query logic inside the module that is the budget and its proof tokens.

Where those five actually get decided, stated exactly, because "above the charge" is true of only two of the three cases:

| | (P0): the two operand-shape refusals | (P1): the three join refusals |
|---|---|---|
| charge would **refuse** | above the charge | above the charge |
| charge would **admit** | above the charge | below it — raised by the join, which is harmless, because the charge admitted and preempts nothing |
| preflight scratch over its ceiling (skip) | above the charge | below it — **and the budget answers first, as it did before the fix.** Unreachable below ~6.75M combined series |
...
- **Sharded** (`PULSUS_CLUSTER` set): every table gets a `Replicated*` engine plus a `_dist` Distributed wrapper (cluster config with `internal_replication = true`). Sharding keys are chosen for read locality, and every table in a signal family uses the byte-identical expression:
  ...
  - Logs family (`log_samples`, `log_streams`, `log_streams_idx`, rollup): **`cityHash64(fingerprint)`** (never `rand()`). The key hashes the column rather than being the column because a `Distributed` sharding key must evaluate to an integer type ClickHouse accepts and `UInt128` is not one — the bare column creates and then refuses every insert (`Code: 53`, issue #498, schemas.md §7). Label→fingerprint lookups still fan out (any shard may hold matching streams), but because index and samples co-shard, the fingerprint sets each shard produces are *local* to that shard's samples — intersections, joins, and per-series aggregation push down shard-locally, and only reduced results cross the network.
  - `trace_spans`, `trace_attrs_idx`, `trace_recent`, `trace_error_spans`: `cityHash64(trace_id)` — a trace is whole on one shard; span-level intersections are shard-local, and the two derived trace tables sit on the shard that holds the spans they are written from.
...
- `/metrics` exposes Prometheus metrics across four implemented families (names are exact):
  - **Ingest** (`pulsus_ingest_*`) — bridged from the writers' atomics on scrape; present only on `writer`/`all` roles (a reader-only process fills no writer slot, so these series are absent). Per-table (label `table` ∈ `log_samples`, `log_streams`, `log_patterns`, `metric_samples`, `metric_series`, `metric_metadata`, `metric_hist_samples`, `trace_spans`, `trace_attrs_idx`): `pulsus_ingest_rows_total`, `pulsus_ingest_bytes_total`, `pulsus_ingest_flushes_total`, `pulsus_ingest_flush_latency_nanoseconds_total` (nanoseconds verbatim; `flushes_total` is the divisor for a mean), `pulsus_ingest_retries_total`, `pulsus_ingest_inflight` (gauge), `pulsus_ingest_spool_write_failures_total`. Per-signal (label `signal` ∈ `logs`, `metrics`, `traces`): `pulsus_ingest_queue_bytes` (gauge), `pulsus_ingest_backpressure_total`, `pulsus_ingest_spool_poison_total`, `pulsus_ingest_spool_uncertain_total`, `pulsus_ingest_rejected_total`. Registration/cache (logs+metrics signals): `pulsus_ingest_registrations_total`, `pulsus_ingest_registration_cache_hits_total`, `pulsus_ingest_registration_cache_misses_total`, `pulsus_ingest_collisions_total`, plus `pulsus_ingest_metadata_upserts_total{signal="metrics"}` and `pulsus_ingest_patterns_dropped_total{signal="logs"}`. Registration-backfill (label `backlog` ∈ `log_streams`, `metric_series`, `metric_metadata`, `trace_attrs_idx`): `pulsus_ingest_backfill_{enqueued,dropped,retries,healed,abandoned}_total` and `pulsus_ingest_backfill_pending` (gauge). Errors are attributed per **signal + error-class** (the counters above), not per ingest protocol — all protocols funnel into the same per-signal atomics.
  - **Label cache** (`pulsus_label_cache_*`) — reader/all: `pulsus_label_cache_series_count`/`_oversize`/`_age_ms` (gauges), `pulsus_label_cache_hits_total`, `pulsus_label_cache_misses_total{reason}`, `pulsus_label_cache_refreshes_total`, `pulsus_label_cache_refresh_failures_total`.
  - **Query eval gate** (`pulsus_query_eval_*`) — reader/all: `pulsus_query_eval_permits_limit`/`_permits_available`/`_in_flight`/`_waiting` (gauges), `pulsus_query_eval_contended_total`, `pulsus_query_eval_wait_nanoseconds_total`.
  - **Search wire domains** (`pulsus_traceql_search_wire_domain_saturations_total`) — reader/all: one counter, labelled `field` in `trace.durationMs`, `trace.startTimeUnixNano`, `span.startTimeUnixNano`, `span.durationNanos` and `bound` in `above`, `below`. Incremented once per rendered integer moved into its wire field's domain on `GET /api/traces/v1/search` and its `GET /api/search` alias. A `below` sample means a stored value was negative, which no mounted ingest route can produce.
- **Not yet exposed:** per-*protocol* ingest error attribution (errors are per signal + error-class today); on-disk spool **size / file-count / backlog** gauges (only the `pulsus_ingest_spool_*_total` class counters exist — there is no spool-size instrumentation); per-API and per-planner-stage query latencies; tier-router segment decisions; tail-session counters.
...
- **Golden semantics tests:** fingerprint vectors; the **pinned upstream PromQL test corpus** (21 `.test` scenario files from the pinned Prometheus release, fetched at test time and checksum-verified against a committed manifest, replayed natively by a test driver that implements the `.test` format) — this is the PromQL compliance gate, run in CI on every engine change; LogQL/TraceQL parser snapshot tests.
...
- **Differential testing:** the M2 differential (issue #33) feeds an identical OTLP corpus, pushed once through a real OTel Collector, to both Prometheus and PulsusDB via one metrics pipeline with two identical `prometheusremotewrite` exporters — the guarantee is eventual sample-set equivalence (a completeness pre-check gates the query matrix), never byte-identical delivery. Per-series per-timestamp value equality is bit-exact on the parsed `f64` except NaN-class (any NaN equals any NaN); a supplementary direct-remote-write micro-case (bypassing the collector, under the same guardrails) covers Prometheus's explicit stale-NaN marker, which OTLP cannot carry. Runs at a ~1,000-series CI-scale tier per-PR and a ~10,000-series full tier at #33 closure, nightly, and on demand. Every comparison row carries a required positive-signal floor (`min_series`) so an all-empty comparison on both backends can never pass silently, and the lane additionally direct-remote-writes a constant-size (~85-series) historical corpus at `now - 26h` — outside the 24h label-cache window but inside Prometheus's out-of-order acceptance window — that is forced onto (and proven, via `X-Pulsus-Explain` and a `pulsus_label_cache_misses_total{reason="out_of_window"}` counter delta) PulsusDB's `metric_series` SQL-fallback read path.
...
| Late/duplicate samples (remote-write retries) permanently inflate tier `sum`/`count` aggregates | A retried push is suppressed at ingest before the insert, by the writer that accepted the original, inside `PULSUS_INGEST_DEDUP_WINDOW` (issue #494) — so the retry reaches neither the sample table nor the tiers. A retry that reaches a *different* writer process is stored, and inflates tiers as before. Independently, at one `(series, millisecond)` raw-path reads answer a stored histogram over any float, else each distinct float value-bit identity; tiers cannot do even that — a documented tier-accuracy caveat, measured in M3 |
...
| Shard-local execution claims (co-sharding → local joins/aggregation) are design intent, not yet observed behavior, for **metrics/traces/profiles**; the **logs family graduated in M1** (issue #16), covering both the stream-resolution/read path and label/tag discovery | M3/M4 distributed benchmarks verify the remaining signal families with `EXPLAIN PIPELINE` + `system.query_log` on a multi-shard cluster before those rows graduate from hypothesis; logs graduated on Tier-1 4-shard evidence captured **per stage and per shard, including the coordinator's own local-shard read, verified against a client-computed expected shard roster** (three CODE-review corrections on #16: terminal-stage-only spot check → per-stage capture; excluded coordinator shard → full shard roster; tolerant 1..N shard-count check → exact sharding-key-derived roster, so a pruned shard is *proven* pruned rather than merely absent), see `docs/benchmarks/m1-logs-read-path.md`. **That evidence was captured while the logs sharding key was the bare `fingerprint` column; the key is `cityHash64(fingerprint)` since issue #498.** What survives the change is the execution SHAPE and the method — per-stage, per-shard capture including the coordinator's own read; each stage confined to the shards that own its fingerprints; and an excluded shard proven pruned against a roster computed before the query ran rather than inferred from its absence. What does NOT survive is anything that says WHICH shards, **including whether they are a subset at all**: the three-of-four roster, shard 4 as the pruned one, the per-shard placement and the per-shard row counts. Measured against a live server over the captured fingerprint lists, the ten-fingerprint stage that reached three of four shards under the previous key reaches all four under the current one, so “fingerprint-scoped stages participate on a subset” is itself a superseded figure. All of these are marked where they appear (`docs/benchmarks/m1-logs-read-path.md`). **Re-capturing them has not been done.** It needs the four-shard fixture, the same corpus loaded through the `_dist` wrappers and a rerun of `xtask bench --dist` — Tier-2 latency-at-scale validation for logs is separately tracked by #25 |
