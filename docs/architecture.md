...
- **Cardinality guard:** if a selector matches more than `PULSUS_CACHE_MAX_SERIES` fingerprints, the planner falls back to a SQL JOIN against `metric_series` instead of shipping a huge IN-list. The cache itself is bounded by an active-series window (default: series seen in the last 24h), not by all-time cardinality — this is the scalability answer for high-churn workloads.
...
| Skip-index false-positive rates on log bodies | Tunable bloom sizes; planner records skip-index effectiveness metrics to guide defaults |
...
| Shard-local execution claims (co-sharding → local joins/aggregation) are design intent, not yet observed behavior | M3/M4 distributed benchmarks verify with `EXPLAIN PIPELINE` + `system.query_log` on a multi-shard cluster before the claims graduate from hypothesis |
