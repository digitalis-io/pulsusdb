...
- **The span row also carries its own attributes, and nothing reads them yet (issue #556).** Five aligned arrays on `trace_spans` — `attr_key`, `attr_scope`, `attr_val`, `attr_type`, `attr_num` — hold exactly the attributes that also become `trace_attrs_idx` rows for that span, in the same order. They are written by the ingest path and read by nothing: every attribute query is still served by the index. They exist so the read moves that follow can be reviewed against a table that already holds what they need.
