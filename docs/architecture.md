...
**How far a carrier can be narrowed, and where Rust stops** (round 4's `[medium]`). Two things still produce a join under a substituted context, and neither is a call to the join. One is editing the join's own signature, body and single call site together. The other is leaving the join untouched and adding a *sibling* dispatcher beside it that consumes the carrier itself and joins under the caller's matcher — measured, it compiles — because the carrier's consuming exit is `pub(super)`, i.e. visible to the whole enclosing module rather than to the join alone. That cannot be narrowed further where the carrier lives: a visibility may name only an **ancestor** of the item's own module, so an exit defined in the budget module can be restricted to that module or to one of its ancestors and to nothing else (both measured: naming a sibling module is `E0742`; making the exit private stops the join itself compiling, `E0624`). Moving the join dispatch inside the budget module so the exit could be private would *shrink* the set of code that may unpack a carrier, from the file to that module — it would not empty it, since a sibling within that module could then do what a sibling beside the join does now. It is not taken: a smaller surface rather than a closed one, bought by putting result-shape query logic inside the module that is the budget and its proof tokens. So the bound stated here is a bound on the **call** — no caller can substitute the context *by calling the join*, every argument route being a compile error — and substituting needs a second unpacking site, which is a new function in the diff rather than a use of the mechanism. That last step, like editing the mechanism itself, is what review of a diff is for; no in-language device prevents it, here or anywhere.
