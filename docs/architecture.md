...
- **The span row carries its own attributes, and the phase-2 attribute condition reads them (issues #556, #557).** Five aligned arrays on `trace_spans` — `attr_key`, `attr_scope`, `attr_val`, `attr_type`, `attr_num` — hold exactly the attributes that also become `trace_attrs_idx` rows for that span, in the same order. The batch hydration statement reads them through one predicate column per condition: `arrayFirstIndex` over `(attr_key, attr_scope)` locates the element the span's attribute resolves to, and the condition's value test is applied to that element. `trace_attrs_idx` still serves **phase 1** — the candidate generator — and the `select()`/aggregate value reads, which issue #558 is the part that moves.
