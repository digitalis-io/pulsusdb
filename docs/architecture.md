...
**The worked (P) case.** `post_agg::combine_binary_capped` (issue #290). All five of the LogQL binary funnel's semantic refusals are class (P), and the ordering is structural rather than conventional: `decide_binary` MOVES both operands — and the operator and matching clause they are decided under — into a value with no accessor, and `Ledger::acquire_binary` is the only way to get any of them back. An operand therefore cannot reach the join except through a charge the decision preceded, and it cannot be joined under a matcher it was not decided under; skipping the decision, conditioning it, deciding one pair and charging another, or swapping a decision's halves are all compile errors rather than review findings.
