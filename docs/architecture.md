...
- **Traces:** span, resource, instrumentation-scope, and span-event attribute keys are stored **verbatim** in the attribute index — TraceQL addresses them by their OTel names (`span.http.status_code`, `resource.service.name`, `instrumentation.<key>`, `event.<key>`). The index carries a `scope` discriminator (`'resource'`, `'span'`, `'instrumentation'`, `'event'`, or the reserved `'event:intrinsic'` — the last three added by issue #192) alongside each verbatim `(key, val)` pair, so `resource.foo`/`span.foo`/`instrumentation.foo`/`event.foo` remain separable even when the same key appears at multiple scopes. Span-event `name`/`timeSinceStart` intrinsics live under the dedicated `event:intrinsic` scope so a sender attribute can never collide with an intrinsic row.
