...
FROM (SELECT trace_id,
             ...
             arrayFilter(x -> has(a_par, x.4) AND NOT has(a_ids, x.1), b_spans) AS b_hit,
             arrayConcat(b_hit, arrayFilter(y -> has(arrayMap(x -> x.4, b_hit), y.4) AND NOT has(arrayMap(x -> x.1, b_hit), y.1), a_spans)) AS hit
      FROM tqd_g1.spans
