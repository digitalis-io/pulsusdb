WITH (SELECT (min(start_ns), max(end_ns))
      FROM tqd_g1.traces WHERE trace_id = toFixedString(unhex('9E0AE95131B5BEDBEEA2C9EB5234F1EC'), 16)) AS ext
              ...
              WHERE day >= toDate(fromUnixTimestamp64Nano(ext.1)) AND day <= toDate(fromUnixTimestamp64Nano(ext.2))
                AND resource_id IN (SELECT DISTINCT resource_id FROM tqd_g1.spans
                                    WHERE (intDiv(start_ns, 300000000000), trace_id) IN
                                          (SELECT (k, toFixedString(unhex('9E0AE95131B5BEDBEEA2C9EB5234F1EC'), 16)) FROM (SELECT arrayJoin(range(intDiv(ext.1, 300000000000), intDiv(ext.2, 300000000000) + 1)) AS k)))
