...
FROM (SELECT trace_id, max(last) AS last, sum(matched) AS matched,
      ...
      FROM (SELECT trace_id, toString(attrs.`a`) AS grp, transform(dynamicType(attrs.`a`), ['String', 'Int64', 'Float64', 'Bool'], ['string', 'int', 'double', 'bool'], if(startsWith(dynamicType(attrs.`a`), 'Array'), 'array', dynamicType(attrs.`a`))) AS grp_type,
            ...
            (SELECT arrayJoin(arrayFlatten(arrayMap((t, ks) -> arrayMap(k -> (k, t), ks), top.1, top.2))))
              ...
              AND (1) AND (dynamicType(attrs.`a`) != 'None')
