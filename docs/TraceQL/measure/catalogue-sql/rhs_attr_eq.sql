    ...
    (SELECT (groupArray(trace_id), groupArray(keys))
     FROM (SELECT trace_id, max(start_ns) AS last,
           ...
           WHERE start_ns >= 1790000000000000000 AND start_ns < 1790000060000000000 AND intDiv(start_ns, 300000000000) BETWEEN 5966666 AND 5966666
             AND (coalesce(multiIf(dynamicType(attrs.`a`) != 'None', attrs.`a`, dynamicType(scope_attrs.`a`) != 'None', scope_attrs.`a`, NULL) = multiIf(dynamicType(attrs.`b`) != 'None', attrs.`b`, dynamicType(scope_attrs.`b`) != 'None', scope_attrs.`b`, NULL), false))
           GROUP BY trace_id
