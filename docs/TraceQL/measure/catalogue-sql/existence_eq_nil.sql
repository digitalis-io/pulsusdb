    ...
    (SELECT (groupArray(trace_id), groupArray(keys))
     FROM (SELECT trace_id, max(start_ns) AS last,
           ...
           WHERE start_ns >= 1790000000000000000 AND start_ns < 1790000060000000000 AND intDiv(start_ns, 300000000000) BETWEEN 5966666 AND 5966666
             AND (NOT ((dynamicType(attrs.`a`) != 'None' OR has((SELECT groupArray(resource_id) FROM tqd_cat.resources WHERE dynamicType(attrs.`a`) != 'None'), resource_id) OR arrayExists(x -> dynamicType(x) != 'None', events.attrs.`a`) OR arrayExists(x -> dynamicType(x) != 'None', links.attrs.`a`) OR dynamicType(scope_attrs.`a`) != 'None')))
           GROUP BY trace_id
