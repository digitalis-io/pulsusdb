...
FROM (SELECT trace_id, max(start_ns) AS last, count() AS matched,
             arraySlice(arraySort(x -> (x.2, x.1), groupArray((lower(hex(span_id)), start_ns, duration_ns, attrs.`http%2Estatus_code`, multiIf(dynamicType(attrs.`foo`) != 'None', attrs.`foo`, dynamicType(scope_attrs.`foo`) != 'None', scope_attrs.`foo`, NULL), service))), 1, 3) AS spans
      FROM tqd_cat.spans
