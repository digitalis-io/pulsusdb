...
| `PULSUS_PROMQL_MAX_SAMPLES` | `50000000` | evaluation sample budget per query, enforced on the metrics read path (issue #138): counts **fetched sample rows** — every `metric_samples` and `metric_hist_samples` row drained across all of one query's selectors (histogram rows count 1 each; rows are counted pre-dedup/pre-merge, the actual materialization cost) — and aborts the fetch on the first over-budget row, returning `422 query_too_broad` naming this knob. Exactly-at-budget succeeds. Series/label hydration, cardinality probes, and discovery endpoints never charge it (they carry their own caps). **Semantics diverge from Prometheus's `--query.max-samples`** (per-step in-eval peak accounting): PulsusDB drains the whole fetch before evaluation, so the fetched-row count IS the memory event being bounded. Accepted range `1..=50000000000`: values above the ceiling are rejected at config load, since they would make the sample budget unreachable. Scale tuning routes to #25 |
