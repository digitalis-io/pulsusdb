...
The subset of these rows where **Prometheus is wrong on its own terms**,
rather than merely different, is indexed and explained in
docs/reference-defects-we-do-not-copy.md, alongside the Loki and Tempo
defects. That file carries the reasoning and the source citations; the
evidence — the measurements, the captures and the gates — stays here.
...
| `otlp-target-info-span-accepted-points-only` | — | `POST /v1/metrics` | `200` | `POST /api/v1/otlp/v1/metrics` | `200` | Both accept; the difference is which `target_info` samples exist, and it is a property of the two TRANSLATIONS rather than a witnessed end-to-end difference. PulsusDB takes the span over **accepted** data points only, because a point our `Date` domain refuses (1970-01-01 … 2106-02-06) would otherwise place a `target_info` sample in a partition `metric_samples` cannot hold. **Whether any payload reaches this difference against a live reference is NOT established here, and no witness is claimed.** The two conditions pull against each other: the reference would have to admit a point we refuse — one more than about 56 years from now — while `otlp-reference-admission-window` records that it refuses a sample older than `head.maxTime − 60 min` and one too far in the future. So this row states our rule and the reference's, and stops there. Gated on our side by `protocols::otlp_metrics::tests::target_info_span_ignores_rejected_data_points`. **Corrected 2026-09-02 — the superseded sentence, and why it was wrong.** This row used to read: *“The reference computes the emission span with `findMinAndMaxTimestamps` before validation (`metrics_to_prw.go:217`), so a data point it later rejects still stretches the span. … Only reachable on payloads we already reject and the reference does not.”* Its first half is true and was re-read at `v3.13.0`: `findMinAndMaxTimestamps` is called at `metrics_to_prw.go:217`, ahead of the temporality check at `:218-233` and the name build at `:235-239`, each of which joins `errs` and then `continue`s. Its second half **is not observable on the reference at all**, and that is the correction. Any joined error reaches the deferred block at `write_otlp_handler.go:132-138`, which rolls the appender back and commits nothing, and the switch at `:177-189` then answers `400` or `500` — so on a payload the reference rejects, **no** `target_info` sample is stored, and the widened span has no consumer. The early call is therefore not a mechanism this row can rest on. A row asserting a reference defect that the reference's own rollback makes unreachable is exactly the false-claim class this ledger exists to prevent, so the claim is withdrawn rather than restated, and the row is kept for the rule it states about OUR side and for the trail. |
