...
- Tag APIs read only `trace_tag_catalog` — discovery never scans span payloads — and the intrinsic vocabulary (the `intrinsic` scope, the closed `status`/`kind` value sets) is served from the TraceQL grammar with no read at all (issue #475).
