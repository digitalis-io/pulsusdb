...
**Status: design intent, not observed behavior.** Co-sharding is necessary but not sufficient — Distributed plans can still merge at the initiator or ship large sets if the generated SQL doesn't cooperate. The fan-out table above graduates from hypothesis to guarantee only when the M3/M4 multi-shard benchmarks confirm it with `EXPLAIN PIPELINE` and `system.query_log` (which shards aggregated, what crossed the network); those snapshots then join the CI regression set.
...
**Latency targets (warm cache, p95):**
