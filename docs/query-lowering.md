...
| `Select { fields }` (`ast.rs:1024`) | any → same shape, wider `cols` | every field resolves in `cols`. **No exactness precondition** — projecting a column onto rows the evaluator will drop is harmless | `cols` unchanged; the fields become an evaluator-owned projection | conditional on resolution, **over the accepted payload set only** | *none* here; a left join would need an ADR 0008 clause that does not exist — [query-to-sql.md](query-to-sql.md) open question 4 |
