...
Underneath them, `is_pushable_line_filter` (`plan.rs:3797`) carries a doc comment that states the
...
five call sites across three files: `plan.rs:1710`, `plan.rs:1732`, `plan.rs:3771`,
...
**The same answer lowered is three statements, not one: 3 round trips.** The compiled generator is
the first; the window-bounded hydration read survives lowering, because `spanSets[].matched` and
...
read is the third and stays residual in **every** chain, because the root summary is read
trace-wide with no time predicate (§3.1's `Emit` row, §5). The attribute condition sends no
...
`compile_line_filters` (`crates/pulsus-read/src/logql/plan.rs:3763`) emits — not against emitted
...
exactly the rule `compile_line_filters` breaks at today (`plan.rs:3778`). Break B's panic traces to the unchanged
...
(`crates/pulsus-read/src/traces/exec.rs:118`). The accepted worst case is bounded by one key-range
...
`== phase1 generator[0] ==`, `== phase2 hydration (sample batch) ==`,
...
loop named for what it is: `BATCH_TRACES = 32` (`crates/pulsus-read/src/traces/exec.rs:118`).
...
that compiles a generator, not by the part whose contract is that no SQL moves. The cases are a
frozen, named exception, asserted as an EQUALITY by
`traces_search_plan_parts::the_generator_fan_out_exception_is_exactly_these_two`, so another
cannot join them silently.

**Two since #560.** `structural_descendant` (`{ resource.service.name = "checkout" } >> { status =
error }`) left the exception when `{ status = error }` began reading `trace_error_spans`: its two
generators now read different tables, so its second part carries `Cut::DisjointSources` like the
other two-source cases. Over the 75 committed search goldens, eleven send two phase-1 generators;
nine read two tables and two (`nested_boolean`, `structural_sibling`) read one.
...
statements, **both reading `trace_attrs_idx`**; `structural_sibling.sql` is the same shape
(`structural_descendant.sql` was the same shape against `trace_spans` until #560 moved its second
generator to `trace_error_spans`). Two SQL parts, and no cut in
...
reads — is owed by the part that compiles a generator, and the remaining two cases are frozen as a named
...
|---|---|---|---|---|---|
| `Source` — `SpansetExpr` (`ast.rs:99`) | — → `Spans` | none; lowers by §2.4's lattice. The seed statement reads one of four sources: `trace_spans`, `trace_attrs_idx`, `trace_recent` (the time-range superset) or `trace_error_spans` (`{ status = error }`) — the last two since #560 | n/a — the seed is always applied. An unlowerable leaf contributes `1` and clears `exact` | **always lowers, possibly partially**. No aggregate is pushed onto `trace_recent` or `trace_error_spans`: `aggregate_having_sql` accepts only the first two sources | *none*, unless the selector is a disjunction over two sources — then `Cut::DisjointSources` (§2.7.4) |
| `Hydrate` (synthesised, `traces/compile.rs:187`) | any → same shape | **never lowers** (`No(NotYetLowered)`) — the batch hydration read is a second statement over a different source keyed by this statement's result, and no SQL form has been written that would put it INTO the seed statement. That is what `NotYetLowered` says here, as against `Never`: nothing about the read is impossible, only unwritten | **none — the identity.** The read adds rows the evaluator consults; it rewrites no column's provenance and narrows no predicate. The row asserts the identity rather than leaving the exemption silent | never lowers | **`Cut::SourceHandoff`** (§2.7.2) — source `trace_spans` (hydration), key `trace_id`, `SeedBound::Config { reader.traceql_max_candidates }` |
...
- **`Emit` is `Never`, and a lowered TraceQL search is three statements, not one.** The root summary
  ...
  (`crates/pulsus-read/src/traces/exec.rs:465`), so every search response needs it. That is exactly
  ...
  the generator plus that one plus the root read. §1 and §4 count **3** since
  [#557](https://github.com/digitalis-io/pulsusdb/issues/557) made the attribute condition a
  predicate column on the hydration read; §9.2 measured **4**, before that change, and its dated
  paragraph says so. The hops diagram still counts 2 and says so on its own face, until part 8
  redraws it.
...
that key inside the window is read. `GenClass::AttrKeyScan` (`filter.rs:100`) already names this
...
- **The negated physical leaf `{ name != "x" }`**, which this document listed under §3.3 until
  ...
  `spans_generator_for` (`filter.rs:2695`), which always returns `GenClass::SpanScan` with the
...
**Lowered** — **three statements**: the lowered generator below, the window-bounded per-batch
hydration read — which since [#557](https://github.com/digitalis-io/pulsusdb/issues/557) carries
the selector's one attribute condition as a predicate column rather than sending a read of its own
— and the same winners' root read the evaluator still owns because `Emit` is `Never` (§3.1). The
last two are unchanged from today, so only the first is shown in full:
...
was wrong twice over.** `unwrap_vector_aggs_into` (`plan.rs:2890`) descends the spine and
...
in `crates/pulsus-read/src/logql/plan.rs` above `mod tests` (`plan.rs:4243`)**, which is 25 sites at
...
- `plan.rs:1108` (`QuerySpanTooLong`), `plan.rs:1139` and `plan.rs:2248` (`InvalidStep`),
  `plan.rs:3075`, `plan.rs:3253`, `plan.rs:3269` (`QueryTooBroad`) — **request parameters and
  resource guards, not a link payload.** They refuse the request before any chain exists.
- `plan.rs:3312` — one `reject` closure returned from three conditions (`plan.rs:3320`,
  `plan.rs:3329`, `plan.rs:3338`; those three are call sites, not constructions, so they are not
...
- `plan.rs:3388`, `plan.rs:3393`, `plan.rs:3402` — the same three range-aggregation arity
...
- `plan.rs:3696` (`ContradictoryMatchers`) and `plan.rs:3726` (`EmptyMatcherSet`) — the
  ...
  **plans** — so `plan.rs:3726` was not reached by the obvious candidate and is excluded on the
...
| link | accepts → produces | precondition to lower | residual state effect | disposition | continuation |
|---|---|---|---|---|---|
| `LineFilter(lf)` (`ast.rs:134`) | `Lines` → `Lines` | `body` provenance resolves to a SQL expression — `Stored`, or `Computed(e)` from `decolorize`/`unpack` (§5) — **and** `is_pushable_line_filter(lf)` (`plan.rs:3797`: no `ip()` alternative) | **clears `exact`**: it removes lines in the evaluator, so the SQL result is a superset | conditional | *none* |
...
- **`compile_line_filters`' `break`.** It does two things: it skips a non-pushable filter, and it
  `break`s at `LineFormat | Decolorize | Unpack` (`plan.rs:3778`). The first is the
  ...
  **parser** still lowers, because *"parsers read but never rewrite the line"* (`plan.rs:3750`), so
  `body` stays `Stored`.
...
(`crates/pulsus-read/src/traces/exec.rs:148`) and `reader.logql_scan_budget_bytes` already do across
...
| `Exact` | hydrated rows | `max_result_bytes` (`crates/pulsus-read/src/traces/exec.rs:155-163`) plus the `HYDRATION_BYTE_BUDGET` retention counter (`exec.rs:148`) — today's mechanism, unchanged |
...
Corpus C1, one request, driven serially exactly as the phase-2 loop (`exec.rs:2075`) and its three
serial reads (`exec.rs:2306`, `2054`, `2079`) drive it. Byte columns are raw `system.query_log`
...
**Dated, 2026-09-18: this section and §9.2b measure the statement set as it stood before issue
[#557](https://github.com/digitalis-io/pulsusdb/issues/557).** Every figure above and below was
taken when an attribute condition sent a membership read of its own, so the lowered request was
four statements and its round-trip count was `1 + 2·ceil(k/32) + 1`. Since #557 the condition is a
predicate column on the hydration read: the same query is **three** statements and
`1 + ceil(k/32) + 1`, and the `phase-2 membership` and `lowered membership` rows name a statement
the engine no longer issues. The figures are not restated against the new statement set — they are
the record of what that read cost, and `xtask/src/bench/traces_lowering.rs` still renders both
statements so the artefact stays rebuildable.
...
**The root read is why the lowered side is 4 and not 1 in this measurement.** It is not an
artefact of the measurement: `Emit` is `Never` (§3.1) because the root summary is trace-wide and
unwindowed, so no chain removes it; and the window-bounded hydration read and the membership read
survived lowering because `spanSets[].matched` and `spanSets[].spans[]` are written
unconditionally. Since [#557](https://github.com/digitalis-io/pulsusdb/issues/557) the membership
read is gone — the condition is a column on the hydration read — so the same request is 3; this
section's figures are the 4-statement measurement and are dated as such in §9.2. The earlier
revision of
...
(`crates/pulsus-read/src/traces/exec.rs:176`), set in `search_settings` (`:3086`) and inherited by
...
(`crates/pulsus-read/src/traces/exec.rs:5664`) is what keeps 4,096 shipped: it asserts that the
...
shipped `compile_line_filters` (`crates/pulsus-read/src/logql/plan.rs:3763`) transcribed as the
...
`search_settings_pin_the_layer_1_budget_contract` (`crates/pulsus-read/src/traces/exec.rs:5664`)
...
`map_trace_generator_error` (`exec.rs:808`) classifies code 241 first, and `read_error_parts`
...
`map_trace_read_error` turns 158 into `TooBroadReason::TraceScanBudgetRows` (`exec.rs:709`) and
...
(`map_trace_generator_error`, `crates/pulsus-read/src/traces/exec.rs:808`) and the request answers
...
(`crates/pulsus-read/src/traces/exec.rs:176`). `use_query_condition_cache=0` is in the instrument so
...
shipped `BATCH_TRACES = 32` loop sends (`crates/pulsus-read/src/traces/exec.rs:120`). **`<the 32>`
...
measured on the phase-1 generators (`148 = 25 + 123`, `traces_search_plan_parts.rs:55-61`),
...
- `traces_graph/clustered_local_join.sql` and `traces_graph/single_node.sql`, one join line each,
  ...
  called from `crates/pulsus-read/src/traces/exec.rs:1802` and nowhere else.
...
`NotASearchLinkLower::capability`, `crates/pulsus-read/src/traces/compile.rs:1396-1404` — so they
...
`is_pushable_line_filter` (`crates/pulsus-read/src/logql/plan.rs:3797`) — `plan.rs:3771`,
...
links (`Hydrate`, the three indexed phase-2 reads, the membership predicate column and the two
trace-wide co-loads), and **§7.1's
...
`crates/pulsus-read/src/traces/exec.rs:2077`, but stated nowhere in the prose), `renders 20 rows`,
...
The citations pointing at an empty line are `crates/pulsus-read/src/logql/plan.rs:1655` (in `docs/query-lowering.md`), `crates/pulsus-read/src/traces/exec.rs:2077` (in `docs/query-lowering.md`), `crates/pulsus-read/src/traces/metrics_sql.rs:627` (in `docs/query-lowering.md`), `metrics_sql.rs:951` (cited from 2 documents), `search_plan.rs:1200` (in `docs/query-lowering.md`), `search_plan.rs:1328` (in `docs/query-lowering.md`), `search_plan.rs:1890` (in `docs/query-lowering.md`), `traces/exec.rs:117` (cited from 2 documents).

The citations the rule answers differently for two occurrences of are .

The citations where the fallback answers a file the citing prose does not describe are `exec.rs:2983-2989` in `docs/query-lowering.md`, `exec.rs:3023` in `docs/query-lowering.md`, `exec.rs:808` in `docs/query-lowering.md`. Each is named with its reasoning in `REVIEWED_FALLBACK_DIVERGENCES`, and the test prints them when it runs.
