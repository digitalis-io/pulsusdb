...
|---|---|---|
| `\|= "text"` | `body LIKE '%text%'` | *emitted today*, `predicate.rs:914` via `escape.rs:93`. `%`, `_` and `\` inside the search text are escaped so they match themselves |
| `!= "text"` | `NOT (body LIKE '%text%')` | *emitted today*, `predicate.rs:521` |
| `\|~ "re"` | `match(body, 're')` | *emitted today*, `predicate.rs:918`. Not anchored: a LogQL line filter searches for a substring |
...
**SQL today** — one statement, `sql.rs:538`. Not anchored, and **not** underscore-escaped: `_` is an ordinary character in a regular expression (`predicate.rs:918`, `escape.rs:156-163`).
...
**SQL today** — one statement, `sql.rs:538`. `ch_like_contains("")` renders `'%%'`; the case is pinned at `predicate.rs:1091`.
...
**SQL today** — none. The pattern is compiled **before any read**, at the point the predicate is built (`predicate.rs:918` through `escape.rs:150-155`), so an uncompilable pattern is a `400` at planning time rather than a ClickHouse failure part-way through a query.
