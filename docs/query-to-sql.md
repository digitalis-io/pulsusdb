...
**Nothing in the test suite reads a claim in part 5, §5.1 or part 7.** Measured by perturbation on
2026-09-09 rather than read off the code: falsifying a value in §5.1 — `5400` changed to `9999` —
and, separately, deleting §5.1 in full, each left both suites that open this file green,
`cargo test -p pulsus-read --test query_lowering_doc_gate` at 5 passed and
`--test logql_pattern_expr_matrix` at 12 passed. The one check that does reach this file reaches
**positions**, not claims: `logql_pattern_expr_matrix::the_sites_dataset_is_regenerated_not_retyped`
reddens when a `<name>`-shaped literal moves line, which it did during this change, and
`crates/pulsus-read/tests/logql_pattern_expr_sites.tsv` is regenerated rather than edited when it
does. So every sentence in those parts is held true by a reader and by nothing else.
