...
Response `200`: `{"fields":[{"label":"…","type":"…","cardinality":N,"parsers":["json"|"logfmt",…],"jsonPath":["…",…]},…],"limit":N}`. Every per-field object and the zero-field body are byte-exact against the reference; the **array order is not**, and deliberately so — see the first bullet.

- `fields` is **sorted by label, ascending**. The reference emits it in Go map iteration order at both the single-response and the sharded-merge build sites, with no sort before marshaling, so its order is unspecified by the Go spec and randomised per iteration by the runtime — *it cannot reproduce its own order between two runs on identical input*. There is therefore no order to mirror, and PulsusDB pins a deterministic one, the same treatment every irreproducible reference tie in this repo gets. Registered as `detected-fields-array-order-pinned` in docs/benchmarks/logs-differential-ledger.md, which carries the source citations. The SET of fields is reference-exact; a client must key on `label`, never on position.
