...
**For LogQL this direction is now measured position by position and checked in** (issue #246): `crates/pulsus-read/tests/logql_regex_accept_matrix.rs` puts every class above to each of the sixteen LogQL constructs that carry a regex — on the digest-pinned v3.7.4 reference and through PulsusDB's own `parse → plan → compile` chain — and its live leg re-measures the reference half in CI. Two things it adds to the table. The `\u{263A}` row's *LogQL in process only* is exact and the reason is `label_replace`, the one LogQL site that rewrites a pattern before compiling it; `[a--b]` behaves identically there, which is why §9.4's closing sentence about it needed the correction it now carries. And there is a class this table does not have: **a malformed regex in a BARE `variants(...)` variant's own line filter is served here and refused by the reference** — and only while that filter is PUSHABLE, and only while the variant is BARE. PulsusDB validates a bare variant's discarded prefix through the pipeline compiler, which skips a pushable line filter entirely (its regex is validated on the SQL-rendering path, and a discarded prefix renders no SQL); put the same filter after a `line_format` and the pushdown is cleared, the filter is compiled, and both sides refuse it. Wrap the same variant in a vector aggregation — `variants(sum by (env) (count_over_time({app="x"} |~ "(" [5m]))) of (...)` — and its whole pipeline is live (issue #397, the reference's own type switch), so the filter is compiled and both sides refuse that too. Everything measured is enumerated in `logql-regex-accept-surface-divergence` (docs/benchmarks/logs-differential-ledger.md) and owned by issue #400.
