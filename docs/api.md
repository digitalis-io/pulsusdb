...
**For LogQL this direction is now measured position by position and checked in** (issue #246): `crates/pulsus-read/tests/logql_regex_accept_matrix.rs` puts every class above to each of the fifteen LogQL constructs that carry a regex — on the digest-pinned v3.7.4 reference and through PulsusDB's own `parse → plan → compile` chain — and its live leg re-measures the reference half in CI. Two things it adds to the table. The `\u{263A}` row's *LogQL in process only* is exact and the reason is `label_replace`, the one LogQL site that rewrites a pattern before compiling it; `[a--b]` behaves identically there, which is why §9.4's closing sentence about it needed the correction it now carries. And there is a class this table does not have: **a malformed regex inside a `variants(...)` variant's own line filter is served here and refused by the reference**, because PulsusDB validates a variant's discarded prefix through the pipeline compiler, which does not compile a line filter's regex. Everything measured is enumerated in `logql-regex-accept-surface-divergence` (docs/benchmarks/logs-differential-ledger.md) and owned by issue #400.
...
- **For LogQL, one narrower claim IS proved** (issue #246), and it is a claim about the reference's error taxonomy rather than about patterns. Every one of the sixteen `ErrorCode` constants the reference's vendored regex parser declares (`vendor/github.com/grafana/regexp/syntax/parse.go:28-48 @ v3.7.4`) is either raised by a named pattern in `crates/pulsus-read/tests/logql_regex_accept_matrix.rs` or carries a stated unreachability reason there — `ErrInternalError` and `ErrInvalidCharClass` are declared and never raised anywhere in that package; `ErrInvalidUTF8` is refused by the LogQL scanner before the regex parser sees the byte (`pkg/logql/syntax/query_scanner.go:264 @ v3.7.4`, measured); `ErrLarge` is pre-empted by the repeat-product cap at `parse.go:434-437` (measured) and otherwise needs a ~33M-rune pattern. The test fails if a code has neither. **This does not make the PATTERN set closed** — a taxonomy is not a grammar, and the classes above are still the ones we found rather than all the ones that exist.
