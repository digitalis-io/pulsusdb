...
|---|---|---|
| 1 | `0.001048576` (1 ms) | `0.000001024` (1 µs) |
| 2 | `0.000001024` (1 µs) | `0.000016384` (16 µs) |
...
histogram is drawn everywhere a user has seen one. This is order ONLY:
every `__bucket` label, every tally and every count is identical, so a
client that reads the label — rather than indexing the array by position,
which the reference's order does not support anyway — sees no difference.
