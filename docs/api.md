...
**Value divergence: an unknown string escape silently changes the pattern (issue #246, owned by #400).** Everything else in §9.3 and §9.4 is an accept-versus-reject difference. This one is not, and it is worse. A LogQL double-quoted string here is lexed by `scan_double_quoted` (`crates/pulsus-logql/src/lexer.rs:322-331`), which handles an **unknown escape by dropping the backslash and keeping the character**. So `"\xff"` — which Go's unquoting turns into one 0xFF byte — becomes the three ASCII characters `xff`, and `{app=~"\xff"}` compiles a regex that matches lines containing the text `xff`. On the reference the same query is either a `400` (wherever the pattern reaches its regex parser) or a matcher for byte 0xFF (at the `NewFastRegexMatcher` sites, which never parse a plain literal). **At those sites both systems answer `200` and match different lines**, so no accept/reject table can see it — including the §9.3/§9.4 measurement, which scores verdicts only. The class is not limited to `\x`: any escape Go defines and this lexer does not takes the same path. The verdict half is pinned in `crates/pulsus-read/tests/logql_regex_accept_matrix.rs` and the decoded value itself by that file's `the_parsed_pattern_value_is_committed_where_the_escape_changes_it`; the fix is issue #400's.
