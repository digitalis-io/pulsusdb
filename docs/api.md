...
- **The escape Go does NOT define — an ordinary accept-surface divergence, and it is in none of the tables above.** `\d`, `\w`, `\s`, `\0`, `\q` and the rest are a `400` on the reference at the LEXER, before any regex parser is reached: measured, `{app=~"\d+"}` and `{app=~"\0"}` both answer `parse error at line 1, col 7: invalid char escape`, as do `{app="x"} |~ "\d+"` (col 14) and `{app="x"} | regexp "(?P<c>\d+)"` (col 20) — so it applies to every construct rather than to particular positions. PulsusDB accepts all of them and compiles the backslash-stripped text (`d+`, `0`, `q`). This half is **absent from the 792-point measurement and from all nine of its divergence classes**, and the reason needs its qualifier: every *other* pattern there is a regex body that the fixture escapes on the way in, so it cannot carry a string escape at all. Exactly one point is passed through unescaped — the `\xff` probe, which exists to measure the value half — and it is not in this half either, because `\x` is an escape Go **defines**. Do not read §9.3's row as covering the reject half.
