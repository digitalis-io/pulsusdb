...
**Which value a span *has*, when it carries a key more than once or at more than one scope (issue #537).** A span has exactly **one** value per attribute: the first stored element within the highest-precedence scope present, taking scopes span → resource → event → link → instrumentation. **Shipped for the condition and for the value a matching condition projects** (issue #557): a span storing `span.k = "y"` then `span.k = "x"` does **not** match `{ span.k = "x" }` and **does** match `{ span.k != "x" }`, and a span storing `resource.k = "x"` alongside `span.k = "y"` does not match `{ .k = "x" }`. The independent value reads — `select()`, `avg()` and a `by()` key — still render whichever entry the aggregate reaches, which is not stable across merges; issue #558 is what moves them, and until it lands `select(span.k)` on a span that repeats `k` may show a different element from the one the condition matched. **Two scopes are carved out, because they are not single-valued**: a span carries one `event.<key>` per EVENT and one `link.<key>` per LINK, so a condition on `event`/`link` matches when **any** element matches and its negation when every one does. The reference's own two paths disagree here (its value reader takes the first match, its pushed-down condition accepts any), which is why this is a divergence we take deliberately rather than a defect to fix; the evidence, the affected shapes and what it means for the differential corpus are in `docs/benchmarks/traces-differential-ledger.md`, entry `traceql-attribute-resolves-to-one-element`.

**What an UNSCOPED condition reaches.** `{ .k = "x" }` resolves through the five attribute scopes — span, resource, event, link, instrumentation — and takes the first one present. It never reaches a writer-reserved intrinsic scope, so a span whose one event is named `evQ` does not match `{ .name = "evQ" }` and does match `{ event:name = "evQ" }`. The reference's filter reaches span and resource only; we are wider on purpose, for the two reasons recorded in `docs/benchmarks/traces-differential-ledger.md`, entry `traceql-unscoped-attribute-scope-reach` — what an unscoped attribute means, and the fact that the §4.3 tag route already advertises five scopes.
