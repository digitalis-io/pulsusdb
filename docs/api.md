...
- **The escape Go does NOT define — an ordinary accept-surface divergence, and it is in none of the tables above.** `\d`, `\w`, `\s`, `\0`, `\q` and the rest are a `400` on the reference at the LEXER, before any regex parser is reached: measured, `{app=~"\d+"}` and `{app=~"\0"}` both answer `parse error at line 1, col 7: invalid char escape`, as do `{app="x"} |~ "\d+"` (col 14) and `{app="x"} | regexp "(?P<c>\d+)"` (col 20) — so it applies to every construct rather than to particular positions. PulsusDB accepts all of them and compiles the backslash-stripped text (`d+`, `0`, `q`). This half is **absent from the 792-point measurement and from all nine of its divergence classes**, because every pattern there is a regex body that is LogQL-quoted on the way in and so carries no undefined string escape. Do not read §9.3's row as covering it.
