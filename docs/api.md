...
**Terminal `sort`/`sort_desc` ordering.** A terminal `sort`/`sort_desc` sets the wire order of the `vector` result: ascending/descending by value, with equal values ordered by label set ascending — a PulsusDB determinism pin, not a mirror of the reference, which orders on the value alone through an unstable sort and so specifies no order among equal-valued samples. Registered as `sort-tie-order` in docs/benchmarks/logs-differential-ledger.md. A client must key on the value and the label set, never on a sample's position within a run of equal values.
...
| Repetition of a repetition | `a**`, `a*+`, `a++`, `a?*`, `a{2}{3}`, `a*??`, `a{2,3}+`. **`a**` is the one that matches everything**: the crate reads it as `(a*)*`, which matched every subject probed (`""`, `a`, `b`, `:`, `101`, an emoji), so a line filter carrying it returns the whole stream | Rust accepts | in process |
| An unrecognised POSIX class name | `[[:foo:]]`, `[a[:zzz:]]`, `[[:^foo:]]` | Rust accepts | in process |
| Non-RE2 group heads | `(?x…)` extended mode, `(?u…)`, `(?i-u:…)`, `(?R)` — a line-terminator flag the Rust crate has and RE2 does not, so `(?R)a` is read as the ordinary pattern `a` rather than being refused. **Correction (issue #400, measured 2026-08-10):** this row used to say `(?R)` makes the pattern "silently match everything". It does not — `(?R)a` matches `"a"` and not `""`, `"b"` or `"x\r\ny"`. The construct that matches everything is the repetition-of-a-repetition row's `a**` | Rust accepts | in process |
