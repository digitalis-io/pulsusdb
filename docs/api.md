...
| An invalid-UTF-8 **escape** in the pattern's string literal | `"\xff"` — one 0xFF byte in the reference's pattern, which its parser refuses; our lexer decodes the same escape to U+00FF, a valid `char`, so the pattern compiles and matches something else | Rust accepts | LogQL, and only where the pattern reaches a parser: the line filter, `\| regexp` and `label_replace`. The reference *also* serves it on the selector, label filters and `drop`/`keep`, which short-circuit a plain literal before parsing it (`vendor/github.com/prometheus/prometheus/model/labels/regexp.go:56-72 @ v3.7.4`) — those positions agree, for a different reason |
