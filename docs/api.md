...
- Responses: `200` with OTLP partial-success message when applicable; `429` on backpressure. On `/v1/logs` only, a request carrying **no log records at all** is `422` rather than an empty `200` — the log receivers' stream-less-push rule, §8.2.
