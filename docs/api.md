...
**Per-stream label bounds (M6, both log receivers — issue #374).** Every stream a log push carries is bounded exactly as grafana/loki 3.7.4 bounds it at its distributor (`pkg/distributor/validator.go:157-199`), and the checks run in that order, so a stream breaking several reports the first: **at most 15 label names** (`entry for stream '{…}' has N label names; limit 15`), **label names at most 1024 bytes** (`stream '{…}' has label name too long: '…'`), **label values at most 2048 bytes** (`stream '{…}' has label value too long: '…'`), **no repeated label name** (`stream '{…}' has duplicate label name: '…'`). Each is a whole-request **400** whose plain-text body is that message. Two rules come with the bounds and decide the accept surface: a label with an **empty value** is neither counted nor length-checked (upstream drops empty labels before validating), and **`service_name` does not count** toward the 15 — so the effective rule is "at most 15 labels other than `service_name`". A stream carrying **no entries** is not validated at all, matching upstream. The bounds apply to `POST /loki/api/v1/push` (both encodings) **and** to OTLP logs (`POST /v1/logs`, `/otlp/v1/logs`), where they are charged on the canonicalized resource attributes — upstream validates its OTLP logs endpoint through the same distributor seam, so this is parity rather than an extra. A duplicate label name is only reachable on the protobuf `labels` literal: a JSON `stream` object and OTLP resource attributes both collapse a repeat before the check, exactly as upstream. Statuses were measured case-by-case against the digest-pinned `grafana/loki:3.7.4` container; the one message difference and the partial-write difference are recorded in `docs/benchmarks/logs-differential-ledger.md` under `ingest-label-bounds`.
