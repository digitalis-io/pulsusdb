...
- `cardinality` is **exact** over the sampled-and-retained values (see the retention model above), where the reference reports a p14 HyperLogLog **estimate**: the estimate equals the exact count for every distinct-value count ≤ 5327 and first diverges at 5328 (no endpoint-reachability bound is claimed — see the ledger entry) — the registered `detected-cardinality-exact-not-estimated` ledger entry (docs/benchmarks/logs-differential-ledger.md), captures in `crates/pulsus-read/tests/golden/detected_cardinality/reference_divergence.tsv`.
...
**String escapes in a LogQL pattern: one root cause, two divergences (issue #246, both owned by #400).** A LogQL double-quoted string here is lexed by `scan_double_quoted` (`crates/pulsus-logql/src/lexer.rs:322-331`), which knows `\n`, `\t` and `\r` and handles **everything else by dropping the backslash and keeping the character**. Go's string grammar knows a great deal more, and rejects what it does not know. That single difference produces two divergences, and only one of them is in this section's tables.

- **The escape Go DEFINES and we do not — a value divergence, invisible to any accept/reject table.** `"\xff"` is one 0xFF byte to the reference and the three ASCII characters `xff` here; `"\101"` is `A` there (Go octal) and `101` here. Measured: `{app=~"\xff"}` and `{app=~"\101"}` are both `200` on the reference and both `200` here, **matching different lines**. The reference *does* reject `\xff` wherever the pattern reaches its regex parser (the line filter, `| regexp`, `label_replace`), but at the `NewFastRegexMatcher` sites — the selector, label filters, `drop`/`keep` — it never parses a plain literal, so both sides serve it. That verdict half is §9.3's last row and is pinned by `crates/pulsus-read/tests/logql_regex_accept_matrix.rs`, whose `the_parsed_pattern_value_is_committed_where_the_escape_changes_it` also pins the decoded value; the matching behaviour is pinned by nothing there, because that file scores verdicts.
- **The escape Go does NOT define — an ordinary accept-surface divergence, and it is in none of the tables above.** `\d`, `\w`, `\s`, `\0`, `\q` and the rest are a `400` on the reference at the LEXER, before any regex parser is reached: measured, `{app=~"\d+"}` and `{app=~"\0"}` both answer `parse error at line 1, col 7: invalid char escape`, as do `{app="x"} |~ "\d+"` (col 14) and `{app="x"} | regexp "(?P<c>\d+)"` (col 20) — so it applies to every construct rather than to particular positions. PulsusDB accepts all of them and compiles the backslash-stripped text (`d+`, `0`, `q`). This half is **absent from the 792-point measurement and from all nine of its divergence classes**, and the reason needs its qualifier: every *other* pattern there is a regex body that the fixture escapes on the way in, so it cannot carry a string escape at all. Exactly one *pattern* is passed through unescaped — the `\xff` probe, which exists to measure the value half — and it occupies 18 of the 792 points, one per position: 43 quoted patterns × 18 positions = 774, plus those 18. None of the 18 is in this half either, because all of them carry the same `\x`, which is an escape Go **defines**. Do not read §9.3's row as covering the reject half.

The two halves can bite the same spelling from opposite directions: a user writing `"\d+"` intending the regex `\d+` gets a `400` on the reference and the literal text `d+` here, neither of which is what they wrote. Both are recorded on issue #400; the PromQL side of this same escape family was examined once before, in `docs/decisions/0002-promql-parser-selection.md:110`.
