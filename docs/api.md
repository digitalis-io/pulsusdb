...
**Which value a span *has*, when it carries a key more than once or at more than one scope (issue #537, scheduled).** Today an attribute condition is satisfied by **any** matching entry on the span: a span storing `span.k = "y"` then `span.k = "x"` matches `{ span.k = "x" }`, and a span storing `resource.k = "x"` alongside `span.k = "y"` matches `{ .k = "x" }` — while `select(span.k)` on the same span renders whichever entry the aggregate happened to reach, which is not stable across merges. **That pair is scheduled to change together**, to the rule the trace storage design settles: a span has exactly **one** value per attribute — the first stored element within the highest-precedence scope present, taking scopes span → resource → event → link → instrumentation — and every operation uses it, the filter, the negation, `select()`, `avg()` and a `by()` key alike. Under the new rule both spans above stop matching, and `{ span.k != "x" }` starts matching the first. The reference's own two paths disagree here (its value reader takes the first match, its pushed-down condition accepts any), which is why this is a divergence we take deliberately rather than a defect to fix; the evidence, the affected shapes and what it means for the differential corpus are in `docs/benchmarks/traces-differential-ledger.md`, entry `traceql-attribute-resolves-to-one-element`. **This paragraph describes a decision, not shipped behaviour**: until the storage change lands, the "today" sentence above is what this route does.
