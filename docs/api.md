...
- **`POST /v1/traces` bounds the bytes one request may expand into, at a fixed 256 MiB.** It is not configurable and it is not `PULSUS_INGEST_QUEUE_BYTES`, which is the separate queue bound that still applies after admission. The estimate is taken before a single row is materialized, so an over-budget request is refused without ever paying for the expansion it describes. From issue #556 on, a span's **attribute arrays** are charged against it too — each attribute now costs one array element and a second copy of its rendered key and value — so a request whose expansion is dominated by attributes and which sat just inside the bound before is now refused. A breach is the whole-request `400` with `google.rpc.Status.code = 3` described above; the alternate ingest receiver shares the same bound but not the same envelope (§8.2).
...
- With `X-Pulsus-Explain: 1`, `data.explain = {"result_type","routing":{"chosen":"rollup"|"raw","reason":"..."}|null,"stages":[{"name","sql","note"|null},...]}` is added alongside `data.stats`. Since issue #492 the object may carry one **additive** fourth key, `plan` — the compiled plan's shape. It is **omitted entirely when absent**. Since issue #492 the **TraceQL search route carries it on every request**, because every search compiles a plan; the logs route still omits it, so an explain response from there is byte-identical to one from before the key existed. Since issue #548 the **metrics route carries an additive `plans`** — an ARRAY of the same plan object, one entry per selector-list position that reads — and it too is **omitted entirely when empty**, which it is on every other route and on a metrics query that sends no statement (`time()`, or a selector whose name matchers exclude its concrete metric name). The keys above have not moved on any route, and a response WITHOUT the header is unchanged on all three.

**The `plan` key's complete shape** (issue #492): an ordered list of `parts` — each part either one SQL statement or work in our own process, with the value set crossing between parts named, typed and bounded — plus one `links` entry per chain link, so every link in the user's pipeline can be traced to the part that runs it. `issue` is `once`, `per_seed:chunks` or `per_seed:keyset`; `yields` is `candidates`, `exact` or `reduced`; `cut` says why a part is its own statement and is `null` only on a part that opens the plan. `seed.from` is a **list** of part indices, because a seed can be a merge: a TraceQL search whose selector disjoins across two tables opens with one statement per table and hydrates their merged candidate set, so that part's `from` is `[0, 1]`. It holds one index whenever one statement produced the values. The example below carries **every key the renderer can emit** — a real response carries only the keys its own plan has:
...
"plans": [{
  "parts": [
    {"kind": "sql", "name": "metric_samples", "issue": "once", "cut": null,
     "seed": null, "yields": "candidates"},
    {"kind": "sql", "name": "metric_hist_samples", "issue": "once",
     "cut": {"why": "disjoint_sources",
             "sources": ["metric_samples", "metric_hist_samples"]},
     "seed": null, "yields": "candidates"},
    {"kind": "engine", "links": [1]}
  ],
  "links": [
...
**Which value a span *has*, when it carries a key more than once or at more than one scope (issue #537).** A span has exactly **one** value per attribute: the first stored element within the highest-precedence scope present, taking scopes span → resource → event → link → instrumentation. **Shipped for the condition, for the value a matching condition projects (issue #557), for every independent value read (issue #558) and — since issue #559 — on the metrics routes and the search route's `| by()` cardinality preflight as well, so a search and a `rate()` over one filter select the same spans**: a span storing `span.k = "y"` then `span.k = "x"` does **not** match `{ span.k = "x" }` and **does** match `{ span.k != "x" }`; a span storing `resource.k = "x"` alongside `span.k = "y"` does not match `{ .k = "x" }`; and `select()`, `min`/`max`/`sum`/`avg()` and a `by()` key all read the same one element the condition would have tested. A span storing `span.dup = 7` then `span.dup = 5` renders `7` under `select(span.dup)`, is returned by `{ } | avg(span.dup) > 6`, and produces the single group `7` under `by(span.dup)` — before issue #558 all four answered `5`, because the value read took `any(val)` over a `GROUP BY`, which is arbitrary and not stable across merges. **The value and its declared kind come from the same element**, so a span storing `span.n = "abc"` then `span.n = 5` renders `{"stringValue":"abc"}` under `select(span.n)` and is **not** returned by `{ } | avg(span.n) > 0` — the element the rule resolves to has no numeric value at all. **Two scopes are carved out, because they are not single-valued**: a span carries one `event.<key>` per EVENT and one `link.<key>` per LINK, so a condition on `event`/`link` matches when **any** element matches and its negation when every one does. The reference's own two paths disagree here (its value reader takes the first match, its pushed-down condition accepts any), which is why this is a divergence we take deliberately rather than a defect to fix; the evidence, the affected shapes and what it means for the differential corpus are in `docs/benchmarks/traces-differential-ledger.md`, entry `traceql-attribute-resolves-to-one-element`. **Two answers on the metrics routes moved with issue #559 and are worth naming**: over eight spans of which three carry `span.k`, `{ span.k = "x" } | count_over_time()` counts **2** where it counted 3, and `{ span.n != 400 } | count_over_time()` counts **7** where it counted 6 — the second because a span whose `n` resolves to a non-numeric element has no numeric value to equal 400, so the negation counts it. **One refusal also went away**: a metrics attribute condition matching more than 524,288 spans used to be answered `422 query too broad: trace metrics attribute-set budget of 1000000 rows exceeded` once the set it built exceeded its byte ceiling, and now returns `200` with the answer. The scan budget (`reader.traceql_scan_budget_rows`, 50,000,000 rows by default) is what bounds it instead, and past that the reply is the ordinary `422`.

**What an UNSCOPED condition or value read reaches.** `{ .k = "x" }` resolves through the five attribute scopes — span, resource, event, link, instrumentation — and takes the first one present. It never reaches a writer-reserved intrinsic scope, so a span whose one event is named `evQ` does not match `{ .name = "evQ" }` and does match `{ event:name = "evQ" }`. **Issue #558 extends the same chain to the VALUE reads**: `{ } | select(.name)` on that span returns the trace with **no** `name` entry, where before it rendered `evQ` — the old read rendered no scope clause at all, so the aggregate reached every scope the key appeared at. The reference's filter reaches span and resource only; we are wider on purpose, for the two reasons recorded in `docs/benchmarks/traces-differential-ledger.md`, entry `traceql-unscoped-attribute-scope-reach` — what an unscoped attribute means, and the fact that the §4.3 tag route already advertises five scopes.
...
**`X-Pulsus-Explain: 1` (issue #492):** this route answers the header, with the same contract as the logs and metrics routes — one execution, never a second run. The response gains one **sibling key** `explain`, carrying §2.1's shape: `{"result_type":"traces","routing":null,"stages":[{"name","sql","note"|null},...]}`, plus the additive `plan` key, which **this route always carries** (issue #492): every search compiles a plan, and `data.explain.plan.parts` names one entry per statement the request may send plus one for the work done in our own process. `routing` is always `null` here — a search never routes between tables. Without the header nothing changes, and `traces`/`metrics` are byte-identical either way.

**Partial results:** the response returns at most `limit` traces (the top-K under the ordering contract above). Candidate generation and consumption are capped **separately** from `limit`, both at `PULSUS_TRACEQL_MAX_CANDIDATES`: each candidate generator is a top-K-by-recency read of that depth, and the merged candidate stream is evaluated up to that many candidates — so the engine may evaluate up to `PULSUS_TRACEQL_MAX_CANDIDATES` candidates even for a small `limit` (stopping earlier only when no unseen candidate can still enter the top `limit`). The result is **incomplete** — `completedJobs` omitted (zero) against `totalJobs` `1` — whenever any internal bound engaged before natural exhaustion: a candidate generator hit its `PULSUS_TRACEQL_MAX_CANDIDATES` depth, the candidate consumption ceiling was reached with candidates still unconsumed, or a single trace exceeded the 10,000 hydrated-spans-per-trace cap (that trace is evaluated on its truncated span set, never silently reported complete). A complete result answers `{"completedJobs":1,"totalJobs":1}`. Our signal is exact and deterministic where the reference's is a racy shard-arrival count, and it is one-directional: we never report completeness we do not have, but a client reading the pair as "shards outstanding" over-reads `{"totalJobs":1}` when the truncation was a single trace's spanset overflow. The request's `limit` and the returned trace count are no longer echoed back — the caller holds the first and `traces.length` is the second. **A query whose spanset aggregate compiled into the generator statement is charged differently, and the difference is visible** (issue #492 part 4): the candidate generator's depth bound is then charged against the traces that PASS the aggregate rather than against the traces the selector matched, because the database applies the aggregate before the depth cap. So a query that answered `{"totalJobs":1}` can now answer `{"completedJobs":1,"totalJobs":1}` — and can return traces where it returned none. Measured on a 1,000-trace corpus whose 333 qualifying traces are the OLDEST and `PULSUS_TRACEQL_MAX_CANDIDATES` of 400: before, the generator returned its 401-row cap probe, all of it non-qualifying, and the answer was zero traces marked incomplete; after, it returns the 333 qualifying traces and the answer is twenty traces marked complete. The aggregate compiles only for `min(duration)`, `max(duration)` and `count()` over a selector that is one attribute-equality condition — `docs/benchmarks/traces-differential-ledger.md`, `traceql-pushed-aggregate-reaches-older-traces`.
...
| Scan or memory budget exceeded (`PULSUS_TRACEQL_SCAN_BUDGET_ROWS` rows read, read/result byte ceilings, the engine's 256 MiB retention budget, or the phase-1 candidate-generator's `PULSUS_TRACEQL_GENERATOR_MAX_MEMORY_BYTES` memory ceiling). **A query whose spanset aggregate compiled into that generator adds per-group aggregation state to the same read** (issue #492 part 4), and a `by()` before the aggregate multiplies that state by the distinct key values per trace (part 5): measured on ClickHouse 26.3, a 1,000,000-group read costs about 193 bytes per group with no aggregate pushed, about 242 with `max(duration_ns)` and about 534 with `uniqExact(span_id)`. **That extra state does not narrow what is answerable** (issue #492 part 5): when the pushed statement breaches the ceiling the reader releases the failed attempt's charge and re-runs the SAME generator with no `HAVING` — byte-identical to the statement the same selector sends with nothing pushed — so a query that answers `200` without the pushdown answers `200` with it, and `422` is returned only when the bare statement breaches too. A request that fell back says so on the explain surface: `X-Pulsus-Explain: 1` adds a `phase1_candidate_generator_fallback` stage carrying the fallback statement and the note `reasongenerator memory ceiling`. No new knob, no new reason and no new status code — `docs/benchmarks/traces-differential-ledger.md`, `traceql-pushed-aggregate-generator-memory`. Every one of these is too broad to bound, never silently slow or quietly incomplete | `422` |
| ClickHouse read timed out | `504` |
